...
## 2026-09-20 - Math module optimization for array norm calculations
**Learning:** Replaced `float(np.linalg.norm(fn))` with `float(math.sqrt(np.vdot(fn, fn)))` in `src/tools/tour_matching_viewer/force_inspection.py` which provides a significant speedup for calculating the length of small 3D vectors avoiding the internal dispatcher overhead of numpy's linear algebra package.
