...
## 2026-09-20 - Math module optimization for array norm calculations in MuJoCo IK
**Learning:** Replaced `float(np.linalg.norm(offset))` with `float(math.sqrt(np.vdot(offset, offset)))` in `src/engines/physics_engines/mujoco/python/full_body_ik.py` which provides a significant speedup for calculating the length of small 3D vectors avoiding the internal dispatcher overhead of numpy's linear algebra package, especially in computationally hot paths like Inverse Kinematics.
**Action:** Always prefer using mathematical operations combined with numpy's `vdot` or `dot` for simple magnitude and length calculations of small 1-D arrays over `np.linalg.norm` to avoid the overhead of python and numpy function dispatch in tight loops.
