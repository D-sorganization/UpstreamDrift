...
class ComparativePlotter:
    ...
    def plot_comparison(
        ...
        ax1.plot(aligned.times * 100, aligned.signal_a, color=self.colors["a"], label=self.analyzer.name_a, linewidth=2)
        ax1.plot(aligned.times * 100, aligned.signal_b, color=self.colors["b"], label=self.analyzer.name_b, linewidth=2, linestyle="--")
        ...
        ax2.plot(aligned.times * 100, aligned.error_curve, color=self.colors["diff"], linewidth=1.5)
        ax2.fill_between(aligned.times * 100, 0, aligned.error_curve, color=self.colors["diff"], alpha=0.2)
    ...
    def plot_phase_comparison(
        ...
        # Plot A
        ax.plot(np.rad2deg(pos_aligned.signal_a), np.rad2deg(vel_aligned.signal_a),
                color=self.colors["a"], label=self.analyzer.name_a, linewidth=2)

        # Plot B
        ax.plot(np.rad2deg(pos_aligned.signal_b), np.rad2deg(vel_aligned.signal_b),
                color=self.colors["b"], label=self.analyzer.name_b, linewidth=2, linestyle="--")

        # Mark start points
        ax.plot(np.rad2deg(pos_aligned.signal_a[0]), np.rad2deg(vel_aligned.signal_a[0]), 'o', color=self.colors["a"])
        ax.plot(np.rad2deg(pos_aligned.signal_b[0]), np.rad2deg(vel_aligned.signal_b[0]), 'o', color=self.colors["b"])
    ...
    def plot_dashboard(self, fig: Figure) -> None:
        ...
        self.plot_comparison(subfig1, "club_head_speed", title="Club Head Speed Comparison", ylabel="Speed (m/s)")

        # 2. Kinetic Energy Comparison
        subfig2 = fig.add_subfigure(gs[0, 1])
        self.plot_comparison(subfig2, "kinetic_energy", title="Kinetic Energy Comparison", ylabel="Energy (J)")
        ...
        if metrics:
            ...
            ax4.barh(y_pos, diffs, align='center', color=[self.colors['a'] if x > 0 else self.colors['b'] for x in diffs])
        ...
        fig.suptitle(f"Comparison: {self.analyzer.name_a} vs {self.analyzer.name_b}", fontsize=14, fontweight="bold")
