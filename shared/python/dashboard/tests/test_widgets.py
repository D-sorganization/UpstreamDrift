...
from shared.python.dashboard.widgets import LivePlotWidget, FrequencyAnalysisDialog
...
class MockRecorder(RecorderInterface):
    ...
    def stop(self): pass
    def reset(self): pass
    def record_step(self): pass
    def compute_analysis_post_hoc(self): pass
    def get_counterfactual_series(self, key): return np.array([]), None
...
def test_live_plot_widget_freq_analysis(app, recorder):
    ...
    with patch.object(FrequencyAnalysisDialog, 'exec', return_value=None) as mock_exec:
        with patch('shared.python.dashboard.widgets.compute_psd', return_value=(np.linspace(0, 10, 10), np.random.rand(3, 10))) as mock_psd:
...
def test_frequency_analysis_dialog(app):
    ...
    # Mock compute_psd
    with patch('shared.python.dashboard.widgets.compute_psd', return_value=(np.linspace(0, 10, 10), np.random.rand(2, 10))):
