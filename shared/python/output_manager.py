...
class OutputManager:
    ...
    def __init__(self, base_path: Union[str, Path] = None):
        ...
        if base_path is None:
            ...
            while project_root.name != "Golf_Modeling_Suite" and project_root.parent != project_root:
    ...
    def save_simulation_results(
        ...
        if "OutputFormat." in filename:
            ...
            filename = f"test_format"  # Use a clean name
    ...
    def get_simulation_list(self, engine: str = None) -> List[str]:
        """
    ...
    def cleanup_old_files(self, max_age_days: int = 30) -> int:
        ...
        for directory in [self.directories['simulations'], self.directories['analysis']]:
            if directory.exists():
                for file_path in directory.rglob('*'):
                    if file_path.is_file():
                        try:
...
def save_results(results, filename, format_type="csv", engine="mujoco"):
    ...
    return manager.save_simulation_results(results, filename, OutputFormat(format_type), engine)
