...
| 2026-09-08 | #9711 | Articulated forward-kinematics engine (`reconstruct/model/kinematics.py`): generic segment tree with 1-3 DOF typed joints (fixed-axis rotations, inclusive limits; hinge/universal/spherical as Euler compositions), `forward_kinematics` vectorised over frames to landmarks plus joint origins/orientations, analytic `dLandmarks/dq` Jacobian about joint origins, `limit_violations` and DbC validation of shapes, finiteness, limits, parent tree and landmarks. Tests on hand-built chains against closed-form poses, explicit Euler products and central finite differences (epic #9709). |
