...
| 2026-09-06 | #8496 | Reconciled the ground-reaction bibliography of #8493 against original sources in `docs/research/proximal_distal_energy_transfer/data/grf_source_reconciliation.json`: the ground-reaction force, centre-of-pressure, free-moment, segment-power, and contact-model topics map to works already adjudicated in `external_source_review.json` and verified against their own DOI, PubMed/PMC, or publisher record, with per-topic gaps and disagreements recorded as declared scope limits. NotebookLM corpus enumeration remains blocked on manual Google authentication (rechecked 2026-09-06, no stored profile), so no notebook or source identifier is recorded and no generated prose is treated as evidence. Enforced by `scripts/research/proximal_distal_energy/grf_source_reconciliation.py` and `tests/research/test_grf_source_reconciliation.py`. |
