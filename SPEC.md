...
## Optimize 1D Norm Calculations (spec-exempt: micro-optimization)

Replaced `np.linalg.norm` with `np.sqrt(ndarray.dot(ndarray))` for small 1D arrays across multiple modules for a measurable performance improvement.
