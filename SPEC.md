...
## Headless and AI Adapter Unit-Gate Debt Burndown (#8766)

Burn down 18 quarantined test node IDs across headless shared Python, AI adapters, and setup wizard clusters in `scripts/config/unit_gate_quarantine.json`:
- Retired Passing Quarantined Tests: Un-quarantine 18 passing headless tests across `tests/ci/test_ci_infrastructure.py` (1 test), `tests/reinforcement_learning/wave6_rl/test_trajectory_funnel_benchmark_coverage.py` (1 test), `tests/unit/config/test_setup_wizard.py` (2 tests), `tests/unit/engines/pinocchio/test_synthesize_target.py` (1 test), `tests/unit/sidekick/lab/bio/test_c3d_io.py` (1 test), `tests/unit/test_launch_upstream_drift_bootstrap.py` (2 tests), `tests/unit/tools/model_generation/test_rest_api_fastapi.py` (1 test), `tests/unit/sidekick/test_data_io.py` (1 test), and `tests/unit/shared_python/ai/adapters/` (8 tests across Gemini and Ollama).
- Validated System Invariants: Ensure CI environment compatibility, reinforcement learning funnel exports, setup wizard embedding, launcher bootstrap repo resolution, FastAPI model generation parameter parsing, Bio C3D export path validation, sidekick CSV/JSON serialization, and Gemini/Ollama provider adapters execute deterministically in headless CI environments.
- Automated Verification: Run `scripts/ci/check_unit_gate_quarantine.py` ensuring contract adherence (ratchet down from 379 to 361 node IDs across 10 clusters) and execute `pytest tests/ci/test_unit_gate_quarantine_contract.py`.
...
| 2026-09-09 | #9859 | Capture product qualification: mutation-safe last-frame cache, repeatable responsiveness benchmark, product review, and child startup failure recovery (#9857). |
| 2026-09-08 | #9858 | Capture Rig GUI inverted so the video is the star (#9846): the live view (preview canvas plus record transport) is the `PaneHost` central widget and every control is a dock — step rail and step details left, settings tabs left, playback and results tabbed right, action grid across the bottom — with the log a drawer tabbed behind the actions, shut at start and opened by a **Log** toggle in the header, so it can no longer take stretch from the video. New `panes.py` holds the arrangement; `PaneHost` gained hidden drawers that survive *Reset layout*, tabified pairs, per-dock starting widths and a `STATE_VERSION` so an arrangement saved by the old, differently-named docks is declined rather than half-restored. `StepRail` (#9845) is wired to `_apply_workflow` and `trigger()`, and `WorkflowPanel` keeps only the current step's guidance. The header, status chips, layout bar and record transport now wrap through `FlowLayout` and long text elides (`labels.ElidedLabel`). Measured offscreen: tile `minimumSizeHint().width()` 3276 -> 1737 (#9844) -> 423 px; at 1280x800 the live view holds 656 px against a 240 px control column, where the video docks used to get 68 px. 266 tests in the package. |
...
| 2026-09-08 | #9854 | Capture Rig GUI: a new `step_rail.py` renders the workflow as a compact vertical rail (194 px minimum width) that marks the current step and offers one primary next action plus that step's remaining actions, quoting `workflow.action_hints` for blocked and skipped reasons; built standalone with `action_triggered(str)` for #9846 to wire into the tile. |
...
| 2026-09-08 | #9822 | Capture Rig multiview layout editor (#9812; epic #9818, wave 2): `tools/capture_rig/layout_editor.py` `LayoutEditor` (grid spinners to 4x4, preset menu over built-ins and `LayoutStore` user/session scopes, drag tiles between cells, span +/-, per-tile source/rotate/flip/crop/fit/label, Save as.../Load/Delete behind injectable dialogs, 20-step undo/redo, one `layout_changed` per edit, what/why-disabled tooltips) and `layout_editor_canvas.py` `LayoutCanvas` (QGraphicsView drawing the whole layout through `compose()` from an injectable `frame_provider`, theme-derived colours only). 37 offscreen tests. |
| 2026-09-08 | #9821 | Capture Rig composite (multipicture) video export (#9815; epic #9818, wave 2): `tools/capture_rig/mosaic.py` exports several streams through a `LayoutSpec` as one video (recorded tiles from the playable file, overlay tiles with the observation set's pose and the variants' 3-D tracks), synchronised by frame index plus the manifest's strobe timing offsets, with a `mosaic-clip/1.0.0` sidecar (layout, sources, frame range, speed, provenance); `rig multipicture --layout NAME|PATH` resolves presets, saved layouts (session then user scope) and JSON files; `clips.compare_takes` stitches through the same `write_composite` loop; `multipicture` workflow action with a new `ACTION_HELP` table and `commands.multipicture_command`. 10 tests. |
| 2026-09-08 | #9820 | Capture Rig multiview foundation (epic #9818, wave 1): `layout_model.py` versioned `LayoutSpec` (rig-layout/1.0.0; grids to 4×4; tiles with source, cell span, rotation, flip, crop, fit, label; seven presets) and a pure numpy/cv2 `compose()` parameterised by a theme-derived palette; `layout_presets.py` `LayoutStore` with user and session scopes and provenance-stamped JSON. 104 tests. |
| 2026-09-08 | #9820 | Capture Rig multiview foundation (#9810, #9811; epic #9818): `tools/capture_rig/layout_model.py` adds the versioned `LayoutSpec` (`rig-layout/1.0.0`; up-to-4x4 grid of `Tile`s with source, cell/span, rotation, flips, normalised crop, fit/fill/stretch and labels; validated `to_dict`/`from_dict` naming the offending field; `with_tile`/`without_tile`/`move_tile`; seven built-in presets) and one pure numpy/cv2 `compose()` with theme-derived `Palette` colours that the live preview, playback and composite export will all render through; `layout_presets.py` adds `LayoutStore` (user and per-session scopes, provenance-stamped JSON, read-only builtins, malformed files reported not fatal). 104 new unit tests including pixel-region assertions for every rotation/flip/fit combination and a 16-tile < 50 ms performance floor. |
| 2026-09-08 | #9820 | Capture Rig multiview foundation (epic #9818, wave 1): `layout_model.py` versioned `LayoutSpec` (rig-layout/1.0.0; grids to 4×4; tiles with source, cell span, rotation, flip, crop, fit, label; seven presets) and a pure numpy/cv2 `compose()` parameterised by a theme-derived palette; `layout_presets.py` `LayoutStore` with user and session scopes and provenance-stamped JSON. 104 tests. |
| 2026-09-08 | #9819 | Capture Rig tile on the theme and layout standards (#9816, epic #9818): `tools/capture_rig/styling.py` composes every colour and stylesheet from `get_current_colors()`/`Styles` (signal red/amber/green read from the `Styles` status constants; no literal colours in the package, enforced by `tests/tools/capture_rig/test_theme_compliance.py`); spacing from `LayoutMetrics` (new `RADIUS_SM`, `ICON_BUTTON_WIDTH`, `TRANSPORT_BUTTON_HEIGHT`, `PROGRESS_BAR_HEIGHT`, `READOUT_MIN_WIDTH`); themed header with session line, status strip (cameras bound / recorder / last take) and layout bar (`header.py`); action buttons grouped by workflow step with section labels (`action_grid.py`); `PlaybackPanel` moved to `playback.py`; theme applied in the embed adapter and the standalone window and restyled on `themeChanged`. Builds under the built-in Dark and Light themes. |
| 2026-09-08 | #9809 | Capture Rig tile: live camera preview. `rig/binding.py` is the single enumerate → plan-check → capture-index binding used by the CLI and the tile; `rig/preview_source.py` decodes each camera through ffmpeg + DirectShow by device reference (the recorder's proven path; OpenCV MSMF hangs on the third unit and DirectShow-by-index refuses 1920x1200@60) into 640-wide BGR frames at 12 fps; `tools/capture_rig/preview.py` binds off the UI thread and runs one worker per view. New *Preview cameras* button; *Record* releases the cameras and the preview resumes after the take; lab plan and a fresh `sessions/<timestamp>-take` folder are prefilled. Verified live on the three-view Sonnet rig. |
| 2026-09-08 | #9804 | Restore manufactured-authority binary compatibility while preserving pinned scientific engines; initialize the exact Tools provider in native lanes, regenerate actual deterministic source/provenance evidence, preserve reviewed claim outcomes and numeric evidence during refresh, regenerate the reviewer summary and canonical PDF, protect canonical bytes from generic formatting, and explicitly declare incoming mocap NumPy type aliases so the normal push hook can validate the merged source. |
| 2026-09-08 | #9785 | Optimize array magnitude calculations in motion capture analytics using np.sqrt(np.einsum) in place of np.linalg.norm. |
| 2026-09-08 | #9805 | Camera-subset matching with full provenance and manual annotation (epics #9790, #9791). A session now holds several matches side by side (`variants/<name>/{reconstruct,model}` through `variants.variant_dir`; `reconstruct --observations SET --views a,b --variant NAME`, `--variant` on fit-model/compare-models/kinetics/export); every pipeline JSON is stamped by `motion_capture/provenance.py` (created time, tool version + git SHA, hashed inputs, parameters, `derived_from`) and `rig lineage` / the tile's Provenance tab walk it back to the recordings and the detector plug-in. `reconstruct/model/fit2d.py` fits the articulated model directly to the 2-D keypoints of 1..N views with cameras borrowed from another variant (the single-camera path; < 10 mm from three views, < 20 mm from two on the synthetic rig); `overlay3d` + `rig overlay` draw one or several variants' joints and model on any view including held-out views; `rig compare-variants` reports held-out reprojection, 3-D and joint-angle RMS against a reference (evidence: pairs cost 32-71 mm / 8-25 deg vs three cameras, singles 370-790 px on held-out views). Guided manual annotation (`motion_capture/annotate/`: sparse clicks/skips store, frame-by-frame cursor, `rig annotations-to-observations` with `--merge-with SET` applying corrections) and the tile's Annotate / edit points dialog with an outlier-editing mode. Two defects sparse data exposed are fixed: the model fit read the reconstruction's all-zero unobservable rows as origin points, and frames no camera saw entered the bundle as free variables that dragged the take toward the gauge camera at 1 px RMS. |
| 2026-09-08 | #9723 | Add always-on unit lane in quality-gate workflow and conftest src-pivot guard in repo hygiene tests (#9409). |
| 2026-09-08 | #9774 | Made the web Cross-Engine dashboard reject invalid perturbation configs instead of coercing them with `|| 0`: raw field text is validated against the API bounds (including `dt < t_end`) in the new `ui/src/pages/crossEngineConfig.ts`, per-field messages render under each input, and the Run button stays disabled while any field is invalid. The status poll now has a 10-minute deadline that fails with an actionable message plus a "Cancel study" button, so a dropped or restarted backend task can always be escaped without reloading the page. (#8891) |
| 2026-09-08 | #9762 | Added the bioptim optimal-control layer (`src/shared/python/optimization/ocp/`) driven by UpstreamDrift's own CasADi dynamics through bioptim's custom-model protocol, pip-only and git-pinned, with biorbd out of scope; and fixed the defects that made the dynamic backends unphysical. The URDF bridge accepts per-link inertials and the model provider derives anthropometric ones from GolferModel mass ratios and ClubModel component masses, so the CasADi and Crocoddyl backends stop treating the bridge's `mass=1.0, I=1e-2` conditioning placeholders as physics (#9755). `casadi_backend` gained `build_mass_matrix`, `build_forward_dynamics`, `build_rk4_integrator` (all validated against `pin.crba`/`pin.aba`/`pin.rnea`), a `dynamics_defect` diagnostic and a `transcription="multiple_shooting"` option; the finite-difference path never enforced the ODE between nodes and is now deprecated (#9756). `MapEstimatorOptions` counts sentinel substitutions and can raise on non-finite residuals (#9757), and both MAP solvers gate free parameters through `identifiability.gate_shared_parameters` (#9758). `ci-optional-stack.yml` gained CasADi, Crocoddyl and bioptim legs, which had no CI coverage at all (#9759). `backend_registry` plus ADR-0050 assign each of six optimizer backends a problem class (#9760). Measured in `docs/estimation/bioptim_parity.md`: the finite-difference path reports 109 m/s of clubhead speed with a 2.88 rad dynamics defect, where the bioptim OCP delivers its requested 50 m/s at 0.22 rad. |
| 2026-09-08 | #9754 | Validated every joint convention of `GolfSwing3D_Kinetic` against the MATLAB model's own dataset-generator logs (`reconstruct/model/simscape.py`: axis order, signs and constant frames per joint by alternating Procrustes over all hypotheses, 620 frames, every joint explained to <= 2.7e-4 rad, evidence in `docs/motion_capture/evidence/simscape_axes.{md,json}`). The logs settle the chain the guide's prose had reversed (torso revolute at the hip, spine universal between the two trunk sensors, hub rigid in the upper trunk at (0, -0.0508, -0.2438) m, forearm sensor 0.1778 m past the elbow). `golfer-scapula/2.0` places its body frames on the Simscape sensor frames via new constant pre/post joint frames in `kinematics.Joint`, so the 27 shared DOFs are the Simscape start-position angles up to a recorded sign; replaying the logged angles through the Python model lands every body frame on its sensor to <= 1.7e-3 rad and joint positions to <= 0.6 mm. The fit's outlier gate no longer rejects more than half of the observations (an unfittable model now reports its honest RMS and ranks last in compare-models). (#9714) |
| 2026-09-08 | #9728 | Hardened the bunker ball boundary for issue #9542: `SandDelivery` now refuses contradictory exit speed/vector pairs at construction and copies list-supplied exit vectors so post-construction mutation cannot invalidate the frozen record; the `to_post_impact_state` boundary carries explicit `ExitVectorProvenance` labels; and `PostImpactEnvelope` wraps the flight handoff with the validity verdict, F0 tier, per-group frames, the proper `HEAD_FRAME_TO_FLIGHT_TRANSFORM`, a schema version, and a SHA-256 source digest with JSON round trip. Reflection rejection was already delivered by PR #9574 and is not redone. |
| 2026-09-08 | #9742 | Migrated the seven launch-monitor analysis handlers (`_run_treatment_ui`, `_run_relationship_ui`, `_run_multivariate_ui`, `_run_model_ui`, `_run_comparison_ui`, `_run_dispersion_ui`, `_run_trend_ui`) onto the #8880 `async_action` worker: each now splits into a widget-free `_compute_*` (optional `WorkerContext`, plain-data return, per-group cancellation checkpoints in the dispersion loop and between the PCA and VIF stages) and a `_present_*` GUI-thread renderer, with the pre-migration synchronous slots kept as `present(compute())`. One shared `AsyncActionBar` disables all seven trigger buttons while anything runs, the embed adapter's `cleanup()` cancels and joins the worker, and `tests/ui/tools/launch_monitor/test_async_actions.py` proves off-thread execution, button disabling, and cancel shortening the work. (#9470) |
| 2026-09-08 | #9739 | Registered the `rtmpose_onnx` pose estimator (RTMPose via `onnxruntime`, SimCC decode, COCO-17/Halpe-26, whole-frame letterbox) behind the estimator registry with `capture_source=False`, the optional `pose-onnx` extra, and pinned official OpenMMLab ONNX model URLs/sizes (digests pending owner approval) (#9648); the shared per-frame video loop and detector-result assembly moved into `pose_estimation.interface` (`estimate_video_frames`, `detection_result`) to satisfy the DRY duplication gate, and the divergence inventory was regenerated for the two new UD-only files. |
| 2026-09-08 | #9719 | Made the `Verify SPEC.md freshness` reminder fail-safe (#9499): the reporting heredoc moved into `scripts/post_spec_reminder.py`, so a posting failure prints its diagnostic into the job log while the `always()`-guarded staleness step still fails the run; covered by `tests/ci/test_spec_check_workflow.py`. |
| 2026-09-08 | #9717 | Extracted the phantom-guard rule-3 closes-issue path-membership check into `scripts/ci/check_phantom_guard_paths.py` with a GitHub API changed-file fallback (env `PR_CHANGED_FILES` or `gh pr view --json files`) for when a shallow base fetch defeats `git merge-base`, failing closed with a diagnostic when neither source is available; workflow rules 1/2/4 are unchanged. (#9091) |
| 2026-09-08 | #9741 | Added runtime sys.modules identity sentinel to isolate and catch worker state corruption from leaking test suites; scoped and restored sys.modules mutations across leak-prone test fixtures. (#9387) |
| 2026-09-08 | #9726 | Fixed the authority lane's `liburdfdom_sensor.so.4.0` import failure (issue #9607): `scripts/research/proximal_distal_energy/articulated_native_runtime.py` now resolves `cmeel.prefix/lib` from the live venv, verifies the locked sonames with an explicit DbC diagnostic, and re-execs the authority profile with `LD_LIBRARY_PATH` prepended before `import pinocchio`; the rolling lane is untouched, and `tests/research/test_articulated_native_runtime.py` pins the resolution. |
| 2026-09-08 | #9744 | Resolved the CLAUDE.md `--no-verify` contradiction (#9494) from the repo side: investigation found no interpreter pin left in `.pre-commit-config.yaml` (`default_language_version: python: python` since #1792/#2720, resolving to the PATH interpreter — Python 3.13.3 on the workstation), so option (a) holds without a config edit; every commit-stage hook plus pre-push `mypy`/`bandit` was verified passing on Windows after a from-scratch hook-environment build, and CLAUDE.md now documents the resolved Windows hook environment with the prohibition intact and no blanket exception. |
| 2026-09-07 | #9738 | Decomposed the sim widget's two oversized functions (#9492): `MuJoCoSimWidget._on_timer` (101 lines) is now a 15-line orchestrator over focused helpers (`_step_dynamic_simulation`, `_apply_control`, `_update_recording_analysis`, `_resolve_live_analysis`, `_render_timer_frame`) and `SimRenderingMixin._add_live_kinematics_overlays` (105 lines) delegates to `_euler_from_matrix`, `_draw_live_pose_overlays` and `_draw_live_screw_overlay`; behavior pinned first in `test_sim_widget_timer_lifetime.py` (RED 14 failed / 24 passed, GREEN 38 passed), the two dated `architecture_budget.json` exceptions for these functions removed, and the C901/PLR0912/PLR0915 noqa suppressions come off the two functions. |
...
| 2026-09-07 | #9483 | Deleted the stale `reports/feature_navigation_gaps/` audit (33 files, last touched 2026-05-15 by `bdffbbbd8`) that described a 15-tile launcher two generations old; today's registry is `src/config/models.yaml` (57 model entries) and the generated launcher manifest carries 50 tiles. Every finding was cross-checked against the current registry and live issues and dispositioned in the deleting PR's body; the live sources of truth for tile discoverability are the registry work (#9412/#9437) and the open accuracy audit #9478, so a hand-maintained, non-generated report cannot rot on main again. |
...
| 2026-09-08 | #9716 | Made the test-only Excel-export and video dependencies reachable from the lock CI actually installs: `openpyxl>=3.1.0` and `imageio[ffmpeg]>=2.31.0` are now declared in the `dev` extra (previously only in `gui-tools` and `pose`, which no CI test lane installs), so `requirements-dev.lock` — compiled with `--extra=dev --extra=gui-test` — will contain them after regeneration and the ~24 `main` import failures (#9533) become real test runs. Lock regeneration itself is delegated to the new dispatch-only `lock-refresh.yml` workflow, which runs the same `make sync-deps` commands as the `ci-standard.yml` dependency-consistency freshness gate on ubuntu + Python 3.12 and opens a PR with the regenerated locks; the lockfiles are intentionally untouched in #9716. |
| 2026-09-07 | #9725 | Declared launcher tile logo families (#9482): a tile may share a logo only within one family derived from the manifest's own fields (`engine:<engine_type>`, else `category:<category>`); reassigned 16 of 50 tiles in `src/config/launcher_manifest.json` to distinct existing SVG assets, breaking the worst reuse (data_explorer 9→5, golf_logo 7→2, all residual sharing declared with rationale in the new gate). Added `scripts/check_launcher_logo_families.py` (fails on cross-family logo reuse, undeclared sharing, missing logo files, or unparsable manifest) plus focused pytest coverage; two verbatim asset copies (`sidekick.svg`, `movement_optimizer_icon.svg`) into `assets/logos/` — no new artwork. |
| 2026-09-08 | #9631 | Pinned the Tools#5048 flattened-install alias contract in this repository's own suite: `tests/unit/repo_hygiene/test_pinned_import_alias_contract.py` loads the vendored `vendor/ud-tools/src/shared/python/import_aliases.py` and asserts, in both a repository layout and a simulated flattened install, that a downstream `src` keeps its own `config` while `sidekick` still aliases to the Tools tree -- the exact predicate defect that let the v2.1.2 wheel resolve `src.shared.python.config` into Tools' unrelated 5-symbol copy and fail to import `get_database_pool_pre_ping`. Verified RED against the pre-#9657 pin `3d93bb2c` (the flattened-layout predicate reported `False` and aliased every shared root) and GREEN at the current pin `eab74a901a`, which already carries the Tools#5049 fix `f8b94bfe` as an ancestor; the pin is therefore left in place rather than rewound, since rewinding would drop Tools #5051-#5057 including the Sentinel ShellTool command-injection fix. The `import_aliases` child copy stays unconverged per #9657's recorded rationale. `AGENT_HANDOFF.md` and the new `docs/development/DEVELOPMENT_LOG.md` entry `DL-#9631` updated in the same commits. |
| 2026-09-08 | #9724 | Gave the `/tools/impact-explorer` route a CI producer (issue #9484): the `rate_of_closure` tile declared `web.mode: route` while the mounted `vendor/ud-tools/src/rate_of_closure/web/dist` was never built, so a clean checkout served the honest fallback. CI Standard's new `impact-explorer-web-build` job builds the bundle from the pinned Tools tree with the fallback's own command (`npm ci`, `npm run build -- --base=/impact-explorer-app/`, node 22 per Tools' `rate-of-closure-web-distribution.yml` authority), verifies the base path, and joins the required `quality-gate` aggregate. `scripts/check_declared_route_producers.py` (focused unit tests included) enforces the general contract: every launcher tile declaring a web route must have a pipeline that produces what its route serves. Wheel/image shipping of the bundle stays an open maintainer decision (#9417). |
...
| 2026-09-08 | #9715 | Articulated model suite (#9709): typed-joint kinematics engine; scapula golfer after the MATLAB 3D model (hub-centred scapula universal + 0.254 m strut to the glenohumeral gimbal); model registry with double/triple pendulums and learnable lengths; whole-trajectory fit with acceleration prior, limits and robust rejection (no jumps); `rig fit-model --model`, `compare-models` (DOF-penalised ranking), `kinetics` (engine-free inverse dynamics + replay check); tile actions/tabs; Simscape-named CSV; TURNOVER.md. |
| 2026-09-07 | #9708 | Reconstruction takes many tape-measured segments (#9707): `reconstruct.measurements` maps everyday names (shank, forearm, upper_arm, thigh, shoulder_width, hip_width, torso) onto both sides, `BundleOptions.measured_lengths_m` (3 mm) beside the hard scale gauge, `--anchor` repeatable in `reconstruct fit` and `rig reconstruct`, Capture Rig *Measured segments* field with tape landmarks, runbook table. |
| 2026-09-07 | #9702 | MotionBERT monocular 3-D lifting evaluated and not adopted (#9683): 103–165 mm aligned MPJPE on the synthetic bundle, depth-dominated widths off by up to 46 %, Human3.6M weights academic-only, 464 MB torch; harness `scripts/research/motionbert_eval.py`, evidence JSON, decision doc. |
| 2026-09-07 | #9685 | Video analysis and coaching tools (#9677): ChArUco boards (`rig board`, `calibrate-intrinsics --board charuco:...`), `rig clip` (event-bounded slow-motion clips with overlay), `rig compare-takes` (side-by-side aligned on an event + metric deltas), joint-angle series in 3-D and 2-D summaries, WSL+xvfb GUI test lane, ADR-0049 (OBS is not a capture layer). |
...
| 2026-09-07 | #9639 | `motion_capture.reconstruct`: DLT triangulation with residuals, covariance and leave-one-out view blame; joint fit of camera poses, per-frame joints and shared bone lengths (rigid segments, prior, symmetry, measured-length scale anchor; analytic sparse Jacobian; IRLS on reprojection/segment terms; per-point rejection, unobservable points, refit); `fit` CLI writing `reconstruction.json`; MediaPipe/BODY_25 to 15-joint layout mapping (C3/C4/C5/C7 of #9619). |
| 2026-09-07 | #9638 | `motion_capture.reconstruct`: robust dynamics-constrained temporal smoother in physical units with listed rejections, reported bound violations and per-frame uncertainty (C6 #9626); per-view cleaning that never imputes into `keypoints_px` (C5 #9625; recall 0.94-0.96 / precision 0.92-0.96 on the synthetic harness); classical ball detector with ranked candidates and at-rest tracking (C1 #9621). |
| 2026-09-07 | #9635 | Added `motion_capture.reconstruct` (C9 #9629): pinhole cameras bridged to ADR-0041 calibration records, rigid fixed-length skeleton with swing motion, synthetic renderer writing ingest-schema observations plus truth (occlusions and outliers listed), reconstruction metrics, `synth` CLI; `record_all` starts its clock when every recorder streams; ingest represents omitted joints as NaN/zero confidence; OpenPose digests pinned; runbook wiring corrected. |
| 2026-09-07 | #9637 | MediaPipe/OpenPose GUI file dialogs use one shared video filter (`pose_estimation/video_files.py`) that admits the capture rig's `.mkv` recordings (#9611). |
| 2026-09-06 | #9632 | Added the `openpose_dnn` estimator (BODY_25 via cv2.dnn), verified model-file downloads shared by MediaPipe and OpenPose resolvers, `motion_capture.rig compare` (coverage/confidence/jitter/agreement per joint), ingest over any registered estimator, and `docs/motion_capture/self_calibrating_pipeline.md` for epic #9619 (C8 #9628). |
| 2026-09-06 | #9615 | `motion_capture.rig`: topology walk resolves each hop with a per-instance PnP property query (the bulk query of #9614 dropped entries and made a camera look missing); `CameraBinding.unserialized` binds the one camera without a USB serial by elimination so it keeps its view on any jack; lab plan and docs updated. |
| 2026-09-06 | #9614 | `motion_capture.rig`: `--mode`/`--views` overrides deriving a named plan, `proxy` command (H.264 mp4 + `proxies.json`), `recorder_note`/`recorder_wall_s` in `recordings.json` with zero-frame recordings classified `blocked`, topology walk 76 s to ~20 s via bulk PnP property queries, lazy ingest import; runbook gains advertised modes, measured single/three-camera throughput and the #9613 stability evidence. |
| 2026-09-06 | #9609 | Added `ingest` to `motion_capture.rig` (#9602, C3 of #9599) and strobe alignment at ingest (#9603, C4): per-view `KeypointObservation` files with estimator/model provenance from a session bundle via the Tasks-API MediaPipe estimator, `raw_confidences` on `PoseEstimationResult`, and reference-clock fields plus `timing_report.json` written beside the untouched per-view clock when the manifest carries strobe timing. |
| 2026-09-06 | #9605 | CI Optional Stack (#9594): the articulated lanes count the pytest summary (`N passed`) instead of per-test `PASSED` lines that `-q` in addopts suppresses, and the rolling lane runs in a job-local venv with `PYTHONNOUSERSITE=1` so `pip check` no longer trips on runner user-site packages. |
| 2026-09-06 | #9606 | Added `record` and `session-check` to `motion_capture.rig` (#9600, C1 of #9599): compressed per-view MJPEG stream-copy into a session bundle (`plan.json`, `recordings.json`, `session_manifest.json`) whose outcome reuses the capture vocabulary, and on-disk bundle validation. |
...
| 2026-09-05 | #9569 | Unblocked the seam cleanup: the pinned Tools tree now answers for retired child copies (program #1505, UD #9406). All 36 actionable rulings in `seam_rulings.v1.json` sat at `pending-cleanup` for want of mechanism, not decision -- every `tools-canonical` ruling says "delete the UD copy and let the pinned tree answer", but nothing put that tree on the import path at runtime, so executing one produced `ModuleNotFoundError`. The mechanism is a `MetaPathFinder` appended to `sys.meta_path`, covering both the canonical `shared.python.X` spelling and the `src.shared.python.X` one this repo actually imports. Appending the tree to `sys.path` was tried and rejected: it exposes every top-level Tools package -- `sidekick`, `chat`, `contracts` -- as importable, which silently flipped availability probes elsewhere (`probe_tools_schema()` went from `unavailable` to `ready`). Extending `src.shared.python.__path__` was also rejected, as a package's `__init__` imports its submodules during execution, before the extension applies. The finder is appended and never inserted, so the standard machinery resolves a present child copy first and no import that resolves today changes; a test pins that it sits after `importlib.machinery.PathFinder`, because pre-empting it would silently flip resolution for the files that still diverge. The mechanism lives in `src/__init__.py` (UD-owned); it cannot live in `import_aliases.py`, which is itself a Tools child copy whose canonical version has no such logic. `deprecation.py` is retired as proof -- byte-identical to the pinned tree, so the fallback returns the same file that was deleted -- moving its ruling to `cleaned`, which switches the drift gate from reporting that path to enforcing it. Inventory: identical 520 to 519, tools-only 444 to 445. Eight further clusters have zero diverged and zero ud-only files and can follow the same pattern. |
...
| 2026-09-03 | #9440 | Retired the first 52 `src/shared/python` child copies that shadowed the pinned Tools tree (`chat_contracts`, `file_watcher`, `logging_pkg`, `notes`, `plot_theme`, `scripting`, `theme` and the loose `cors.py`/`deprecation.py`/`safe_eval.py`/`safe_pandas_eval.py` modules), replacing them with `src/shared/python/_seam_redirect.py` so `src.shared.python.<pkg>` resolves to the canonical Tools package instead of a divergent local fork. This is a correctness change, not tidy-up: because UpstreamDrift imported its own child copies rather than `vendor/ud-tools`, a defect fixed in Tools stayed broken here (#8894). The matching entries in `docs/shared_tools/seam_rulings.v1.json` move `pending-cleanup` -> `cleaned` with `cleaned_in: UD #9406 PR-3`, and the `seam-drift-gate` job enforces that record. Restore any deleted file with `git checkout 365be8778^ -- <path>`. (UD #9406, program #1505) |
| 2026-09-03 | #9497 | Fixed the CI Standard tests (3.12) hang: the cline provider test server now survives `is_available()` probe connections (EOF-only peers are not answered, `OSError` during a reply is swallowed, the writer always closes), so Python 3.12 `Server.wait_closed()` no longer blocks forever on a crashed handler; pinned by two RED→GREEN regression tests plus a bounded `wait_closed()` integration test (#9431). |
...
| 2026-09-09 | #9874 | Chat stream cancellation now reaches the provider transport: `BaseAgentAdapter` gained `_track_stream()`/`cancel_stream()`, the Ollama and OpenAI adapters publish their live streams (both block in an uninterruptible socket read and never poll the cooperative `stop_event`), and `ChatService.stream_response` closes the provider stream before its bounded join and warns if the worker still outlives it. The #9495 regression test now models a non-cooperative provider that only unblocks when its stream is closed. |
| 2026-09-09 | #9875 | Industrial readiness execution index (epic #9539): `src/config/industrial_readiness.json` reconciles the four priority children against `10caddd219ce213a914fa295661929e4fbf1b686` rather than the audit snapshot — U1 (#9477) and U2 (#9407) merged with SHAs, tests and user-visible acceptance evidence; U3 (#8820, dashboard exports still carry no engine/model/run identity) and U4 (#9417, `deploy/` empty and no artifact beyond the wheel) re-confirmed open with a dependency and an ordered narrow-PR plan. `industrial_readiness_loader.py` enforces the contract that keeps the record honest (no completion claim without a 40-char merge SHA, a test path and acceptance evidence; no open entry without an owner and plan; every cited path must exist; every open issue must appear in an acceptance blocker list; `release_status` cannot read `ready` while anything is outstanding, and is `blocked`). `scripts/generate_industrial_readiness_index.py` renders `docs/operations/industrial-readiness-index.md` under a byte-for-byte freshness gate. 25 tests in `tests/config/industrial_readiness/`. |
...
| 2026-09-09 | #9868 | Non-destructive per-view swing trim/crop recipes, original-pixel and source-clock-preserving inference, native preview/editor, downstream timeline regression and visible capture library with notes/archive/storage/import/rename/copy, cancellable scans, panel-owned session navigation, interrupted-rename recovery and separate swing video export; camera initialization/analytics preserve trimmed source timelines, sparse reconstruction reports unavailable summary metrics without blocking model fitting, and generated maps show these integrations; advanced reference work remains in progress under #9849/#9863. |
| 2026-09-09 | #9856 | Generate source-backed capability/workflow maps and searchable offline atlas linked from the launcher (#9850).  Required code-quality CI also checks atlas freshness independently of changed-test selection. |
...
| 2026-09-08 | #9736 | Wired the vendored `spec-rows` merge-driver installer into the documented setup: `scripts/setup_hooks.py` -- this repository's local-automation entry point -- now calls `scripts/install_spec_merge_driver.py`, so `python scripts/setup_hooks.py` registers the `merge.spec-rows.*` config and the per-clone `SPEC.md merge=spec-rows` attribute for anyone running the documented setup instead of the registration staying inert for every contributor here (issue #9476, second comment; `Tools#4818` is the model). A non-zero installer result is logged as an error naming the fix path, not swallowed; the installer docstring now names UpstreamDrift's entry point (`scripts/setup_hooks.py`) instead of Repository_Management's `install_workspace_hooks.py`, a file that does not exist here, closing the "looks installed but never was" trap at the place someone checks first. `tests/unit/scripts/test_setup_hooks_wires_spec_merge_driver.py` drives the real `main()` flow with every side-effecting step stubbed and asserts the installer is invoked exactly once on the resolved repository root, plus that a failed registration is reported; the vendor drift test's pinned divergence is widened from one docstring line to the exact three-line span so the pinned upstream digest is unchanged. Follows #9734 (the re-vendor this behaviour change rides on); the real installer was not executed locally -- it writes clone-global git state -- so registration is validated via the stubbed main flow plus the installer's `--dry-run` smoke. |
| 2026-09-03 | #9472 | Shared off-GUI-thread action mechanism, with the worst-case tool migrated onto it (#8880). Across all of `src/tools/` exactly one file used `QThread` and exactly two ever set a wait cursor; everything else ran heavy compute inline in a `clicked` handler, so the window stopped repainting, the triggering button stayed enabled and a second click queued a second run, and nothing could be cancelled. New `src/tools/async_action.py` generalises the existing `FitWorker`/`RunFitButton` pattern into `run_in_worker`, `WorkerContext` (cooperative `raise_if_cancelled` plus progress reporting), `WorkerHandle` with a bounded cancel-and-join, and an `AsyncActionBar` that disables its trigger buttons for the duration. `simulation_backends_launcher` -- whose sweep ran 24 rollouts at up to the 5000-step horizon, 120,000 integration steps synchronously -- is migrated as the proof, with each action split into a cancellable compute half and a GUI-thread-only present half and the synchronous `run_*` methods unchanged. Remaining tools, the grep ratchet, and relocation into `src/shared/python/ui` are tracked in #9470. |
| 2026-09-08 | #9733 | Fail fast on the uninitialized vendored Tools fallback: `src/__init__.py`'s registration probe now treats a `shared.python` spec that resolves only to UpstreamDrift's own aliased copy as "no Tools present" and raises an actionable ImportError naming `git submodule update --init vendor/ud-tools`, instead of installing `_VendoredToolsFallbackFinder` against a missing vendored tree and livelocking pytest collection in unbounded meta-path `find_spec` recursion. Regression tests (`tests/unit/repo_hygiene/test_src_fallback_fail_fast_9733.py`) simulate the uninitialized probe by monkeypatching the vendored-tree path and the `shared.python` spec, so they never touch the real submodule and never hang; checkouts with the pin initialized keep every existing import path (focused repo_hygiene + api suites fail-identical to the pre-change baseline). |
| 2026-09-08 | #9476 | Re-vendored the corrected `spec-rows` merge-driver files from Repository_Management#1521: `scripts/install_spec_merge_driver.py` (carrying the measured three-state table, the honest tidiness rationale for keeping attribute and config in one place, and the unset-BOTH-keys disarm procedure replacing the withdrawn claim that git aborts a merge when a named driver is unregistered -- the installer stamped that false explanation into every clone's `$GIT_COMMON_DIR/info/attributes`) and `shared_scripts/spec_changelog.py` (byte-identical already; pinned anyway). The repo-wide scan for the withdrawn claim finds only legitimate survivors: the correction narrative quoting it to refute it, true statements about the half-configured `.name`-without-`.driver` state, and the untouched vendored `vendor/ud-tools` tree, which carries Tools' own corrected copies. New `tests/unit/scripts/test_spec_merge_driver_vendor_drift.py` pins SHA-256 digests against the upstream reference so a future silent divergence fails instead of re-shipping a wrong string into every clone; the installer's one deliberate divergence (its docstring will name this repository's wiring entry point in the companion wiring PR) is normalised in the test before hashing. Registration wiring into `scripts/setup_hooks.py` is deliberately a separate behaviour-change PR the issue requests, per its second comment. |
| 2026-09-06 | #9569 | Enforced the documented model_generation preconditions, clearing the densest failure cluster on `main` (53 of 760, program #1505, UD #9474/#9406). The tests assert `ContractViolationError` for empty or missing identifiers; the code raised bare `ValueError`, and its guards only tested `is None` so empty strings passed through. `ContractViolationError` subclasses `ValueError` rather than the reverse, so those raises never satisfied the assertions. Twelve methods now use `require()`, and `BaseURDFBuilder._check_invariants()` is implemented -- joint endpoints must name existing links and names must be unique per kind, checked as a whole-model invariant because a joint may legitimately be added before its child link. Required a guard change: `model_generation` is ruled `ud-canonical` yet exists in the pinned Tools tree, so the child-copy guard reported fixes to UpstreamDrift's own code as forbidden edits while the failing tests sat in UpstreamDrift's own suite; the guard now consults `seam_rulings.v1.json` and exempts ud-canonical clusters, with tests pinning that tools-canonical stays guarded. |
