...
- **Launcher Manifest**: `src/config/launcher_manifest.json` is generated from the single tile registry `src/config/models.yaml` (#9412) and declares discoverable and hidden launcher surfaces, including shared Tools-hosted video/data utilities exposed to UpstreamDrift.
...
| ---------- | ------- | ---------------------- | --------- |
| 2026-09-03 | 1.0.726 | Title-cased the help-page headings and de-pathed a changelog row (#9413). The 47 per-tile help pages used `## See also` (plus `## Method citations`, `## Energy analysis`, `## Kinematic sequence` and `## Jacobian analysis` on the calculation sheet), which `check_document_title_case.py` rejects -- 54 violations across 48 files, all now title-cased with their in-page anchors verified to still resolve. Separately, the 1.0.725 row quoted a deleted docs subdirectory as a backticked path, and `check_spec_paths.py` reads a backticked path in SPEC.md as a claim that it exists, so that row now names the directory in prose instead. |
| 2026-09-03 | 1.0.725 | Dropped the stale `ui/` doc-catalog row (#9413). `1b53a9bf5` deleted the docs subdirectory named by that row as part of the one-tile-registry work but left the row itself in the `docs/index.md` catalog, so `check_doc_catalog.py` reported `docs/index.md has stale ui/ catalog entry` and `doc-governance` was red on every subsequent run of that branch. Removing the row is the whole fix; the directory is genuinely gone and its content is covered by the launcher-tile table the registry generator maintains in `README.md`. |
| 2026-09-03 | 1.0.723 | Marked the help-wiring suite (#9413). `tests/help/test_tile_help_wiring.py` shipped in 1.0.719 without a suite marker, so all 17 tests were net-new unmarked and the suite-marker ratchet (issue #7158) failed. The module now carries `pytestmark = pytest.mark.unit`: every assertion in it is fast, in-process and headless-safe, including the Qt affordance checks, which build widgets on the offscreen platform without a live launcher. |
| 2026-09-03 | 1.0.722 | Extracted `EmbeddedHostWidget._build_tool_widget` (#9413). Attaching the per-tile help affordance in 1.0.719 turned the tab path's and the dock path's shared resolve-context-create-check sequence into a six-line verbatim duplicate, which the DRY duplication gate rejected (`src/launchers/embedded_host.py:355-360` and `:752-757`, 2 occurrences against a baseline max of 1). Both call sites now go through one helper that resolves the tool, injects the launcher context, builds the widget, refuses a `None` widget, and attaches the tile's F1 help. No behaviour change. |
| 2026-09-03 | 1.0.721 | One help page per tile (#9413; closes #8844 #8847). All 47 `ready`/`beta` tiles now point `help:` at their own `docs/help/<tile_id>.md` instead of sharing five generic topics -- `simulation_controls.md` alone had been the help for 16 tiles and `engine_selection.md` for 6, and 19 tiles had no page at all. Each page is derived from the tile's own code and docstrings and carries purpose, inputs and outputs with units, the calculation or engine it uses with a citation, and limitations. Six are marked `status: stub` in their front matter with an explicit `## Unclear` section naming what could not be determined, because guessing would have been worse than admitting the gap: `rate_of_closure` (`maturity: ready`, but no implementation exists in the repo -- the only tracked file matching the name is the tile's logo), `swing_objective_lab` and `golf_simulation_suite` (the first resolves into a Tools provider pack with no `swing_objectives/` subpackage locally; the second has two divergent placeholder code paths and the manifest and embed adapter disagree about which one runs), `tools_calculator_hub` (byte-identical registry launch target to `data_processor`, no adapter and no route, so nothing local distinguishes them), and `biomech_gait` / `biomech_sit_to_stand` (the shell only forwards an exercise filter; the metrics come from sibling model checkouts). The `analysis_tools_api` page is the calculation sheet #8844 asked for -- energy, kinematic-sequence and Jacobian formulas as the code implements them -- and it is where the `MethodCitation` metadata of #8847 finally reaches a user: Putnam 1993 (DOI 10.1016/0021-9290(93)90084-R), Cheetham 2001, McHardy & Pollard 2005 (DOI 10.1136/bjsm.2004.014514) and Hosea 1990, each tied to the result field that carries it, with the two citations that declare no DOI saying so rather than inventing one. The `HELP_RATCHET` introduced with the gate in 1.0.719 is now empty and deleted: `generate_registry_artifacts --check` enforces unconditionally that a `ready`/`beta` tile declares a help page and that the page exists. |
| 2026-09-03 | 1.0.719 | Wired the help system to the tile registry (#9413; closes #8843 #8846). `src/config/models.yaml`'s `help:` key is now the only source of a tile's help page: `src/shared/python/ui/tile_help.py` resolves it, loads it, and attaches the same affordance everywhere -- an `F1` shortcut on every embedded tool widget (both `embedded_host` paths and the three `launcher_simulation` dock-as-tab paths) plus a Help menu built by the previously-dead `build_help_menu` on every window, reached from `SimulationGUIBase.HELP_TILE_ID` for the five engine GUIs. `ContextHelpDock` consults the registry before its legacy rule table, so the dock and F1 can no longer disagree. Two root defects are closed with it: in-app Help opened `docs/user_guide/user_manual.md`, a 124-line index, rather than the 16,794-line `upstream_drift_user_manual.md` (the index stays as a fallback), and `UI_HELP_TOPICS` registered 35 components while `FEATURE_HELP` defined 6 topics, so 11 components' help buttons resolved to `None` -- the nine missing topics (getting_started, docker_setup, wsl_setup, matlab_integration, data_export, urdf_generator, model_explorer, ball_flight, configuration) now cite the documents they front instead of restating them. `generate_registry_artifacts --check` gained a help-coverage gate: a declared page must exist, and a `ready`/`beta` tile must declare one unless it is on the shrink-only `HELP_RATCHET` (19 tiles awaiting a page under #9413), so a new tile cannot ship without help. `tests/help/test_tile_help_wiring.py` (16 tests) is the anti-regression: every declared page exists and loads in the dock, every registered component resolves, F1 and the Help menu are installed and idempotent. `tile_help` lives under `src/shared/python/ui/` rather than `gui_pkg/` because `gui_pkg/__init__` eagerly imports `video_pose_pipeline` -> `pose_estimation` -> `bunkershot3d`, which would have made the launcher's help wiring depend on an optional extra. |
...
| 2026-09-03 | #9475 | Stopped the help wiring holding the Pinocchio GUI hostage to its own type debt. Declaring `HELP_TILE_ID` on the two Pinocchio window modules pulled both into mypy's changed-file set, and `code-quality` failed on 17 pre-existing errors the one-line attribute did not introduce (optional-ndarray assignments and mixin self-argument mismatches). Those two files are reverted untouched; the tile id is declared centrally in `tile_help.WINDOW_CLASS_TILE_IDS` and resolved by `SimulationGUIBase` as a fallback, so the Pinocchio window keeps its F1 help. `HELP_TILE_ID` stays the preferred mechanism for anything new. |
| 2026-09-03 | #9437 | One tile registry: `src/config/models.yaml` drives both launchers, with `maturity`, `surfaces`, `feature_id` and `help` per tile and generated manifest/parity/README artifacts (closes #9412 #8853 #8861 #8863 #8832 #8833). The retired divergence-allowlist parity test under tests/config/ is removed rather than restored: it froze the 21/14 desktop-vs-web split the registry exists to eliminate. To make that deletion reviewable instead of merely blocked, the `tests` guard added by #7368 now delegates to `scripts/ci/check_reviewed_test_deletions.py`, which requires a `scripts/config/reviewed_test_deletions.json` entry naming a replacement test that must exist (`tests/config/test_tile_registry.py`), a tracking issue and an expiry; an unrecorded deletion, an expired record or a missing replacement still fails the job. |
| 2026-09-03 | #9453 | Wired the help system to the tile registry: `help:` is the only source of a tile's page, F1 and a `build_help_menu` Help menu on every tile, in-app Help repointed off a 124-line index onto the real manual, nine missing `FEATURE_HELP` topics added so all 35 registered components resolve, and a help-coverage gate in `generate_registry_artifacts --check` (closes #8843 #8846). |
| 2026-09-03 | #9455 | Extracted `SimulationManager._try_dockable_launch`, clearing the 100-line function budget that the help wiring surfaced on `_execute_local_launch`, and made `attach_tile_help` refuse rather than raise on a widget that cannot carry a shortcut. |
| 2026-09-03 | #9458 | One help page per tile: all 47 ready/beta tiles point at their own `docs/help/<tile_id>.md` instead of four shared topics, six are honest `status: stub` pages for tiles with no reachable implementation, the calculation sheet lands, the `MethodCitation` DOIs reach a user, and the coverage gate's ratchet is deleted (closes #8844 #8847). |
| 2026-09-03 | #9463 | Extracted `EmbeddedHostWidget._build_tool_widget` so the tab and dock paths stop being a six-line verbatim duplicate of each other after the help wiring. |
| 2026-09-03 | #9466 | Marked `tests/help/test_tile_help_wiring.py` as `pytest.mark.unit`; its 17 tests were net-new unmarked and failed the suite-marker ratchet (#7158). |
| 2026-09-03 | #9468 | Dropped the doc-catalog row for a docs subdirectory the one-tile-registry work had already deleted, which had left `doc-governance` red on the branch. |
| 2026-09-03 | #9469 | Title-cased 54 headings across 48 of the new help pages and de-backticked a deleted path in a changelog row that `check_spec_paths.py` read as a claim it still exists. |
