...
| 2026-09-06 | #9588 | Wire split mesh generators into public facade (issue #9584). In `humanoid_character_builder`, wired split generators (`_mesh_makehuman.py`, `_mesh_primitives.py`, `_mesh_smplx.py`, `_mesh_types.py`) into public facade `mesh_generator.py`, adding `solver_status` and `segment_mesh_by_range` to `_mesh_types`, and path-traversal validation to `_mesh_makehuman`. |
