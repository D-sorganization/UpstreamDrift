...
| 2026-09-08 | #9785 | Replaced `np.linalg.norm(..., axis=1)` with `np.sqrt(np.einsum('ij,ij->i', ...))` in `src/motion_capture/reconstruct/analytics.py` and `src/motion_capture/reconstruct/analytics2d.py` to optimize array magnitude calculation. (spec-exempt: micro-optimization) |
<!-- prettier-ignore-end -->
---
