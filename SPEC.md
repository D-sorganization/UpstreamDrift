...
- Replaced `np.linalg.norm(..., axis=1)` with `np.sqrt(np.einsum(...))` in `src/bunkershot3d/solvers/` and `src/bunkershot3d/fields/schema.py` to optimize array magnitude calculation. (spec-exempt: micro-optimization)
