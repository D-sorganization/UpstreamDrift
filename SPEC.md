...
| 2026-09-15 | n/a | Optimized small 1D array norm checks by replacing `np.linalg.norm(arr)` with `math.sqrt(arr.dot(arr))` in `src/engines/physics_engines/mujoco/python/mujoco_humanoid_golf/_mouse_picking.py` and `src/engines/physics_engines/mujoco/python/mujoco_humanoid_golf/sim_rendering_mixin.py`, and `np.linalg.norm(arr) > _TOL` with `arr.dot(arr) > _TOL**2` in `src/motion_capture/reconstruct/cameras.py` for ~2x speedup. (spec-exempt: micro-optimization) |
...
- Replaced `np.linalg.norm()` with `math.sqrt(np.dot())` for small 1D vectors in `src/engines/physics_engines/mujoco/python/full_body_model.py` to bypass linear algebra overhead. (spec-exempt: micro-optimization)
