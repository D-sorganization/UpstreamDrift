...
| 2026-09-07 | #9715 | Articulated golfer model (#9709): `reconstruct.model` typed-joint kinematics engine, scapula-capable spec after the MATLAB 3D golf model (hub-centred scapula universal joint + 0.254 m strut to the glenohumeral gimbal, spine/axial torso, pronation, wrists, legs/head added), whole-trajectory fit with acceleration prior, joint limits and robust rejection (no jumps), `rig fit-model`, tile *Fit model*, Simscape-named `joint_angles_simscape.csv` on export. |
