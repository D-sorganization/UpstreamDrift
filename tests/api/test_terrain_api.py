...
class TestEnvironmentLoading:
    ...
    def test_load_preset_succeeds(self, client: TestClient, preset: str) -> None:
        ...
        resp = client.post("/api/v1/terrain/load", json={"preset": preset})
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_load_invalid_preset_returns_error(self, client: TestClient) -> None:
        ...
        resp = client.post("/api/v1/terrain/load", json={"preset": "moon_surface"})
        assert resp.status_code == 200
        data = resp.json()
...
class TestTerrainQueries:
    ...
    def test_query_returns_200(self, client: TestClient) -> None:
        ...
        client.post("/api/v1/terrain/load", json={"preset": "fairway"})
        resp = client.post("/api/v1/terrain/query", json={"x": 25.0, "y": 100.0})
        assert resp.status_code == 200

    def test_query_response_fields(self, client: TestClient) -> None:
        """Query response has all required fields."""
        client.post("/api/v1/terrain/load", json={"preset": "fairway"})
        data = client.post("/api/v1/terrain/query", json={"x": 25.0, "y": 100.0}).json()
    ...
    def test_query_different_terrain_types(self, client: TestClient) -> None:
        ...
        client.post("/api/v1/terrain/load", json={"preset": "full_hole"})
        # Center should be fairway
        center = client.post(
            "/api/v1/terrain/query", json={"x": 30.0, "y": 100.0}
        ).json()
    ...
    def test_query_out_of_bounds_handled(self, client: TestClient) -> None:
        ...
        client.post("/api/v1/terrain/load", json={"preset": "putting_green"})
        resp = client.post("/api/v1/terrain/query", json={"x": 9999.0, "y": 9999.0})
        assert resp.status_code == 200
...
class TestMaterialListing:
    ...
    def test_list_terrain_types_returns_all(self, client: TestClient) -> None:
        ...
        data = client.get("/api/v1/terrain/types").json()
        assert isinstance(data, list)
...
class TestActiveTerrain:
    ...
    def test_active_terrain_default(self, client: TestClient) -> None:
        ...
        resp = client.get("/api/v1/terrain/active")
        assert resp.status_code == 200
        data = resp.json()
        assert "name" in data
    ...
    def test_active_terrain_after_load(self, client: TestClient) -> None:
        ...
        client.post("/api/v1/terrain/load", json={"preset": "driving_range"})
        data = client.get("/api/v1/terrain/active").json()
