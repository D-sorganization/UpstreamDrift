...
def test_multiple_shooting_options_validation() -> None:
    ...
    with pytest.raises(ValueError, match="Segmented forward batch"):
        MultipleShootingOptions(
            shooting_nodes=(0.4, 0.8),
            segmented_forward_batch=1,  # type: ignore[arg-type]
        )
...
def test_transformed_nodes_and_explicit_acceptance(monkeypatch, solver) -> None:
    ...
    for allowed in [True, False]:
        ...
        def make_accept_fn(is_ok: bool) -> Callable[[np.ndarray], bool]:
            return lambda pred: is_ok
        ...
        opts = MultipleShootingOptions(
            ...
            acceptance=make_accept_fn(allowed),
        )
        result = fit_multiple_shooting(
            target,
            forward,
        ...
        if solver == "least_squares":
            assert result.optimality is not None and np.isfinite(result.optimality)
        else:
...
def test_analytic_pelvis_yaw_jacobian_directional_derivatives() -> None:
    """Verify analytic pelvis yaw Jacobian against numerical finite differences of the full residual."""
    time = np.array([0.0, 0.5, 1.0])
    target_points = np.zeros((3, 2, 3))
    target_points[-1, 0] = [0.0, 0.0, 0.0]  # WaistLeft
    target_points[-1, 1] = [0.2, 0.1, 0.0]  # WaistRight (target angle ~ 26.56 deg)
    target = MarkerTarget(time, target_points, np.ones(2))

    # A 2-parameter model affecting terminal marker position non-linearly
    def seg(theta, clock, state):
        pred = np.zeros((len(clock), 2, 3))
        # Non-zero initial and intermediate points
        pred[:, 0] = [0.0, 0.0, 0.0]
        # x depends on theta[0], y depends on theta[1]
        pred[:, 1, 0] = 0.2 + 0.05 * theta[0]
        pred[:, 1, 1] = 0.05 + 0.1 * theta[1]
        return pred, np.zeros(1)

    def continuous(theta, clock):
        return seg(theta, clock, None)[0]

    def win_jac(theta, clock, state):
        # marker shape: [len(clock), 2, 3, n_theta]
        m_jac = np.zeros((len(clock), 2, 3, 2))
        m_jac[:, 1, 0, 0] = 0.05
        m_jac[:, 1, 1, 1] = 0.1
        end_jac = np.zeros((1, 2))
        return m_jac, end_jac

    yaw_weight = 40.0
    opts = MultipleShootingOptions(
        shooting_nodes=(1.0,),
        pelvis_indices=(0, 1),
        pelvis_yaw_weight=yaw_weight,
        window_jacobian=win_jac,
        finite_difference_step=1e-6,
    )

    # Test at multiple non-zero parameter vectors theta
    ...
    for test_theta in [
        ...
        # Evaluate analytic shooting Jacobian through optimization setup
        # Directional test using finite differences of segment
        pred0, _ = seg(test_theta, time, None)
        obs_term = target_points[-1]
        from src.shared.python.motion_matching.pelvis_yaw import (
            compute_pelvis_yaw_residual_and_derivative,
        )

        m_jac_term = win_jac(test_theta, time, None)[0][-1]
        res0, ana_jac, _ = compute_pelvis_yaw_residual_and_derivative(
            pred_markers_term=pred0[-1],
            target_markers_term=obs_term,
            wl_i=0,
            wr_i=1,
            yaw_weight=yaw_weight,
            marker_jac_term=m_jac_term,
        )
        assert ana_jac is not None
        assert ana_jac.shape == (2, 2)

        for _ in range(4):
            d = rng.normal(0, 1, size=2)
            d /= np.linalg.norm(d)
            eps = 1e-7
            pred_pert, _ = seg(test_theta + eps * d, time, None)
            res_pert, _, _ = compute_pelvis_yaw_residual_and_derivative(
                pred_markers_term=pred_pert[-1],
                target_markers_term=obs_term,
                wl_i=0,
                wr_i=1,
                yaw_weight=yaw_weight,
            ...
            ana_dir = ana_jac @ d
            np.testing.assert_allclose(ana_dir, fd_dir, rtol=1e-5, atol=1e-5)


def test_analytic_pelvis_yaw_empty_and_stale_cache_behavior() -> None:
    """Verify that Jacobian evaluates cleanly with empty, stale, or out-of-order cache."""
    time = np.array([0.0, 0.5, 1.0])
    points = np.zeros((3, 2, 3))
    points[-1, 0] = [0.0, 0.0, 0.0]
    points[-1, 1] = [0.2, 0.1, 0.0]
    ...
    def seg(theta, clock, state):
        ...
        pred[-1, 1] = [0.2, 0.05 + 0.1 * theta[0], 0.0]
        return pred, np.zeros(1)
    ...
    def win_jac(theta, clock, state):
        m_jac = np.zeros((len(clock), 2, 3, 1))
        m_jac[-1, 1, 1, 0] = 0.1
        end_jac = np.zeros((1, 1))
        return m_jac, end_jac
    ...
    assert np.isfinite(fit.pelvis_yaw_diff_deg)
    assert np.isfinite(fit.pelvis_yaw_error_pct)
    # The solver evaluated Jacobian and residual cleanly
    assert eval_count[0] > 0


def test_analytic_pelvis_yaw_slsqp_node_chart_columns() -> None:
    """Verify pelvis yaw Jacobian compatibility with SLSQP and node-chart coordinates."""
    time = np.array([0.0, 0.5, 1.0])
    points = np.zeros((3, 2, 3))
    points[-1, 0] = [0.0, 0.0, 0.0]
    points[-1, 1] = [0.2, 0.1, 0.0]
    ...
    def seg(theta, clock, state):
        ...
        s_offset = 0.0 if state is None else state[0]
        pred[-1, 0] = [0.0, 0.0, 0.0]
        pred[-1, 1] = [0.2, 0.05 + 0.1 * theta[0] + s_offset, 0.0]
        return pred, np.array([s_offset + 0.01 * theta[0]])

    def continuous(theta, clock):
        return seg(theta, clock, None)[0]

    def win_jac(theta, clock, state):
        local_dim = 1 if state is None else 2
        m_jac = np.zeros((len(clock), 2, 3, local_dim))
        m_jac[-1, 1, 1, 0] = 0.1
        if state is not None:
            m_jac[-1, 1, 1, 1] = 1.0
        end_jac = np.zeros((1, local_dim))
        end_jac[0, 0] = 0.01
        if state is not None:
            end_jac[0, 1] = 1.0
        return m_jac, end_jac
    ...
    fit = fit_multiple_shooting(
        ...
        lower_theta=np.array([-5.0]),
        upper_theta=np.array([5.0]),
        initial_states={0.5: np.array([0.0])},
        state_bounds={0.5: (np.array([-1.0]), np.array([1.0]))},
        options=opts,
    )
