...
def test_drake_fit_receipt_passes_g1() -> None:
    """TDD Step 3: Fit receipt passes G1 physical acceptance."""
    ...
    result = fit_full_body_drake(
        ...
        options=DrakeFitOptions(control_mode="knots", max_iterations=10),
    )

    receipt = result.receipt
    assert receipt["schema"] == RECEIPT_SCHEMA
    assert receipt["engine"] == "drake"
    assert receipt["acceptance"]["is_physically_accepted"] is True
    assert receipt["acceptance"]["status"] == "PASSED"

    # Verify G1 quantitative gates
    shared = receipt["metrics"]["shared"]
    assert shared["whole_marker_rmse_m"] <= 0.025
    assert shared["early_marker_rmse_m"] <= 0.012
    assert shared["terminal_marker_rmse_m"] <= 0.035
    assert shared["club_marker_rmse_m"] <= 0.060
    assert shared["pelvis_yaw_rmse_rad"] <= 0.05236
