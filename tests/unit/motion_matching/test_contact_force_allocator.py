...
@pytest.mark.unit
def test_allocator_truthful_rename_soft_trail_and_compatibility() -> None:
    """Verify truthful rename of soft trail-arm penalty and backwards compatibility handling."""
    nv = 10
    actuated = np.arange(6, nv)
    alloc = ContactForceAllocator(nv=nv, actuated_indices=actuated, n_contact_spheres=2)

    # Truthful enum value
    assert AllocationObjective.MINIMUM_TRAIL_ARM.value == "minimum_trail_arm"
    # HARD_ZERO_TRAIL objective exists
    assert hasattr(AllocationObjective, "HARD_ZERO_TRAIL")
    assert AllocationObjective.HARD_ZERO_TRAIL.value == "hard_zero_trail"

    # Compatibility method or string resolution
    assert (
        AllocationObjective.from_string("trail_zero")
        == AllocationObjective.MINIMUM_TRAIL_ARM
    )
    assert (
        AllocationObjective.from_string("soft_trail_zero")
        == AllocationObjective.MINIMUM_TRAIL_ARM
    )
    assert (
        AllocationObjective.from_string("hard_zero_trail")
        == AllocationObjective.HARD_ZERO_TRAIL
    )


@pytest.mark.unit
def test_allocator_hard_zero_trail_mode_locks_torques_to_zero() -> None:
    """HARD_ZERO_TRAIL mode must constrain specified trail joints strictly to 0 N*m."""
    rng = np.random.default_rng(77)
    nv = 12
    actuated = np.arange(6, nv)
    alloc = ContactForceAllocator(nv=nv, actuated_indices=actuated, n_contact_spheres=2)

    j_ground = np.zeros((6, nv))
    j_ground[:3, :3] = np.eye(3)
    j_ground[3:, :3] = np.eye(3)
    ...
    tau_rnea[2] = 600.0
    tau_rnea[6:] = rng.standard_normal(6) * 10.0

    # Trail arm is coordinates 6, 7
    trail_indices = [6, 7]
    res = alloc.allocate(
        tau_rnea=tau_rnea,
        j_ground=j_ground,
    ...
    # In HARD_ZERO_TRAIL, torques at trail_indices must be exactly zero
    tau_full = np.zeros(nv)
    tau_full[actuated] = res.tau_actuated
    for idx in trail_indices:
        assert np.isclose(tau_full[idx], 0.0, atol=1e-5), (
            f"Joint {idx} torque not zero: {tau_full[idx]}"
        )


@pytest.mark.unit
def test_allocator_tau_bounds_not_broken_by_post_solve_correction() -> None:
    """Post-solve projection must never overwrite or violate declared actuator bounds."""
    ...
    tau_rnea = np.zeros(nv)
    tau_rnea[2] = 500.0
    tau_rnea[6:] = 50.0  # wants 50 N*m

    # Tight bounds: [-15.0, 15.0]
    lb = np.full(n_act, -15.0)
    ub = np.full(n_act, 15.0)
    ...
    # tau_actuated must not exceed bounds
    assert np.all(res.tau_actuated >= lb - 1e-6), "tau_actuated broke lower bound!"
    assert np.all(res.tau_actuated <= ub + 1e-6), "tau_actuated broke upper bound!"


@pytest.mark.unit
def test_allocator_enforces_friction_cone_feasibility() -> None:
    """Contact force allocator must enforce Coulomb friction cone feasibility (|f_xy| <= mu * f_z)."""
    nv = 10
    actuated = np.arange(6, nv)
    alloc = ContactForceAllocator(
        nv=nv, actuated_indices=actuated, n_contact_spheres=2, mu_friction=0.6
    )
    ...
    # Moderate horizontal force: 200 N / 600 N = 0.33 <= 0.60
    tau_rnea_ok = np.zeros(nv)
    tau_rnea_ok[0] = 200.0
    tau_rnea_ok[2] = 600.0

    res_ok = alloc.allocate(tau_rnea=tau_rnea_ok, j_ground=j_ground, j_grip=j_grip)
    assert alloc.is_friction_feasible(res_ok) is True
    assert res_ok.max_friction_ratio <= 0.6 + 1e-3

    # Severe horizontal force demand on root: 600 N / 600 N = 1.0 > 0.60
    tau_rnea_bad = np.zeros(nv)
    tau_rnea_bad[0] = 600.0
    tau_rnea_bad[2] = 600.0
