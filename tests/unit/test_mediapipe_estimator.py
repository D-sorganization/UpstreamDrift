...
class TestMediaPipeEstimator:
    ...
    @pytest.fixture
    def estimator_instance(self, mock_mediapipe: MagicMock) -> mediapipe_estimator.MediaPipeEstimator:
        ...
        with patch.object(mediapipe_estimator, "MEDIAPIPE_AVAILABLE", True):
            with patch.object(mediapipe_estimator, "mp", mock_mediapipe):
                 ...
                 with patch("shared.python.pose_estimation.mediapipe_estimator.cv2", MagicMock()):
                     estimator = mediapipe_estimator.MediaPipeEstimator(min_detection_confidence=0.7)
                     yield estimator

    def test_initialization(self, estimator_instance: mediapipe_estimator.MediaPipeEstimator) -> None:
    ...
    def test_load_model(self, estimator_instance: mediapipe_estimator.MediaPipeEstimator, mock_mediapipe: MagicMock) -> None:
    ...
    def test_estimate_from_image(self, estimator_instance: mediapipe_estimator.MediaPipeEstimator) -> None:
    ...
    def test_estimate_from_video(self, estimator_instance: mediapipe_estimator.MediaPipeEstimator) -> None:
    ...
    def test_temporal_smoothing(self, estimator_instance: mediapipe_estimator.MediaPipeEstimator) -> None:
    ...
    def test_joint_angles_calculation(self, estimator_instance: mediapipe_estimator.MediaPipeEstimator, mock_mediapipe: MagicMock) -> None:
        ...
        # Reset all to 0
        for lm in landmarks:
            lm.x = 0.0; lm.y = 0.0; lm.z = 0.0; lm.visibility = 1.0

        # Set specific landmarks
        # right shoulder (12)
        landmarks[12].x = 0.0; landmarks[12].y = 1.0; landmarks[12].z = 0.0
        # right elbow (14)
        landmarks[14].x = 0.0; landmarks[14].y = 0.0; landmarks[14].z = 0.0
        # right wrist (16)
        landmarks[16].x = 1.0; landmarks[16].y = 0.0; landmarks[16].z = 0.0
    ...
    def test_missing_mediapipe(self) -> None:
        ...
        with patch.object(mediapipe_estimator, "MEDIAPIPE_AVAILABLE", False):
            with patch.object(mediapipe_estimator, "mp", None):
                 ...
                 with patch("shared.python.pose_estimation.mediapipe_estimator.logger") as mock_logger:
