...
import pytest
from shared.python.injury.injury_risk import InjuryRiskScorer, RiskLevel, InjuryType
...
def test_low_risk_scenario(scorer):
    ...
    joints = {
        "hip_lead": MockJointResult(20),
        "shoulder_lead": MockJointResult(20)
    }
...
def test_technique_risks(scorer):
    ...
    assert any("early extension" in str(report.recommendations).lower() for r in report.recommendations) # Logic inside recommendation text might differ, checking generic availability logic 
...
def test_category_weighting(scorer):
    ...
    joints = {
        "hip_lead": MockJointResult(50),
        "elbow_lead": MockJointResult(50)
    }
