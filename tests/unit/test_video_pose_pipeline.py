...
from shared.python.pose_estimation.interface import PoseEstimationResult  # noqa: E402
from shared.python.video_pose_pipeline import (  # noqa: E402
    VideoPosePipeline,
...
def test_filter_by_quality(pipeline):
    ...
    results = [
        ...
        PoseEstimationResult(joint_angles={}, confidence=0.1, timestamp=0.1, raw_keypoints={}), # Low confidence
...
def test_is_outlier(pipeline):
    ...
    consistent_results = []
    for i in range(10):
        consistent_results.append(PoseEstimationResult(
            raw_keypoints={},
            confidence=0.9,
            timestamp=0.0,
...
def test_process_batch(pipeline):
    ...
    with patch.object(pipeline, 'process_video') as mock_process:
        mock_process.return_value = MagicMock(valid_frames=10, total_frames=20, average_confidence=0.8)
