...
def test_mujoco_momentum_conservation():
    ...
    # However, TOTAL MOMENTUM of the system should be conserved regardless of elasticity if there are no external forces. 
    # (Gravity is 0).
...
def test_pinocchio_momentum_conservation():
    ...
    # Body 1
    idx1 = model.addJoint(0, pinocchio.JointModelFreeFlyer(), pinocchio.SE3(np.eye(3), np.array([-5,0,0])), "ball1")
    model.appendBodyToJoint(idx1, inertia, pinocchio.SE3.Identity())
    model.addBodyFrame("b1", idx1, pinocchio.SE3.Identity(), idx1)
    
    # Body 2
    idx2 = model.addJoint(0, pinocchio.JointModelFreeFlyer(), pinocchio.SE3(np.eye(3), np.array([5,0,0])), "ball2")
    ...
    # Let's leave q at neutral (origin of joint frame).
