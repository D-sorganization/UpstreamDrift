...
% Open documentation
% Try to find documentation in the docs folder
app_dir = fileparts(mfilename('fullpath'));
repo_root = fullfile(app_dir, '..', '..', '..');
doc_file = fullfile(repo_root, 'docs', 'TABBED_GUI_IMPLEMENTATION_PLAN.md');

if exist(doc_file, 'file')
    open(doc_file);
else
    % Try opening the README instead
    readme_file = fullfile(app_dir, 'README.md');
