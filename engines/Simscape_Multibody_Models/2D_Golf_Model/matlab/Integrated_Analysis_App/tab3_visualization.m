...
% Use the Skeleton Plotter directory for default data files
default_data_path = fullfile(fileparts(mfilename('fullpath')), ...
    '..', '..', 'Skeleton Plotter');

%% Add visualization path
% Add path to SkeletonPlotter and other visualization tools
viz_path = fullfile(fileparts(mfilename('fullpath')), ...
    '..', '2D GUI', 'visualization');
if exist(viz_path, 'dir') && ~contains(path, viz_path)
    addpath(viz_path);
end
