...
class AdvancedGolfAnalysisWindow(QtWidgets.QMainWindow, AdvancedGuiMethodsMixin):
    ...
    def __init__(self) -> None:
        ...
        # Set default model to Full Body (index 4) so it looks like a Humanoid
        # Indices: 0=Chaotic, 1=Double, 2=Triple, 3=Upper, 4=Full, 5=Advanced
        default_index = 4
        if len(self.model_configs) > 5:
    ...
    def _create_control_tab(self) -> None:
        ...
        self.model_descriptions[0] = "Simple driven pendulum showing chaotic behavior."

        self.model_combo.addItem("Golf: Double Pendulum (2 DOF) - Shoulder + Wrist")
        self.model_descriptions[1] = (
            "Basic golf swing with shoulder and wrist joints. Simplest realistic model."
        )
        ...
        self.model_descriptions[3] = (
            "Upper body model with spine rotation and both arms."
        )
        ...
        # Container for Dynamic Mode Controls (Actuators)
        ...
        filter_label.setBuddy(self.actuator_filter_input)
        ...
        # --- Dynamic Mode: Actuator Controls ---
        ...
        self.actuator_scroll_area = QtWidgets.QScrollArea()
        self.actuator_scroll_area.setWidgetResizable(True)
        self.actuator_scroll_area.setHorizontalScrollBarPolicy(
            QtCore.Qt.ScrollBarPolicy.ScrollBarAlwaysOff,
        )

        self.actuator_container = QtWidgets.QWidget()
        self.actuator_layout = QtWidgets.QVBoxLayout(self.actuator_container)
        self.actuator_scroll_area.setWidget(self.actuator_container)

        actuator_header_layout.addWidget(self.actuator_scroll_area, stretch=1)

        # Add actuator grouping to dynamic layout
        dynamic_layout.addWidget(actuator_header)
        control_layout.addWidget(self.dynamic_controls_widget, stretch=1)

        # --- Kinematic Mode: Pose Controls ---
        ...
        pose_header = QtWidgets.QGroupBox("Joint Pose Controls")
        pose_layout = QtWidgets.QVBoxLayout(pose_header)

        # Scroll area for joints
        self.joint_scroll_area = QtWidgets.QScrollArea()
        ...
        self.joint_scroll_area.setWidget(self.joint_container)

        pose_layout.addWidget(self.joint_scroll_area)
        kinematic_layout.addWidget(pose_header)

        control_layout.addWidget(self.kinematic_controls_widget, stretch=1)
    ...
    def load_current_model(self) -> None:
        ...
        try:
            ...
            QtWidgets.QMessageBox.warning(
                self,
                "Model Loading Error",
                f"Failed to load model '{config['name']}':\n{e}\n\n"
                f"Falling back to Double Pendulum model.",
            )
    ...
    def _create_actuator_controls(self, actuator_names: list[str]) -> None:
        ...
        actuator_index corresponds directly to the model's actuator indices (0 to nu-1).
        """
    ...
    def on_model_changed(self, index: int) -> None:
        ...
        # Refresh kinematic controls if active (to reflect reset state)
        if self.sim_widget.operating_mode == "kinematic":
    ...
    def update_body_lists(self) -> None:
        ...
        if hasattr(self, "viz_body_combo"):
        ...
        for body_id in range(1, self.sim_widget.model.nbody):  # Skip world (0)
            ...
            if body_name:
                ...
                if hasattr(self, "viz_body_combo"):
                    self.viz_body_combo.addItem(item_text)
                if hasattr(self, "manip_body_combo"):
    ...
    def on_change_body_color(self) -> None:
        ...
        if not hasattr(self, "viz_body_combo") or self.sim_widget.model is None:
            return

        text = self.viz_body_combo.currentText()
        if not text:
            return
        ...
        if color.isValid():
            ...
            start_geom = self.sim_widget.model.body_geomadr[body_id]
            num_geoms = self.sim_widget.model.body_geomnum[body_id]

            if start_geom >= 0 and num_geoms > 0:
                for i in range(num_geoms):
                    geom_id = start_geom + i
                    self.sim_widget.model.geom_rgba[geom_id] = rgba

                self.sim_widget._render_once()

    def on_reset_body_color(self) -> None:
        """Reset selected body color to default."""
        # This would require storing defaults, for now we just set to gray
        # or we could reload the model? Reloading is safer but slow.
        # Let's just set to a generic gray for now.
        if not hasattr(self, "viz_body_combo") or self.sim_widget.model is None:
            return
        ...
        try:
            body_id = int(text.split(":")[0])
            rgba = [0.5, 0.5, 0.5, 1.0]  # Default gray
            # Apply to all geoms of this body
            start_geom = self.sim_widget.model.body_geomadr[body_id]
            num_geoms = self.sim_widget.model.body_geomnum[body_id]
    ...
    def _on_operating_mode_changed(self, index: int) -> None:
        ...
        self.dynamic_controls_widget.setVisible(not is_kinematic)
        self.kinematic_controls_widget.setVisible(is_kinematic)
