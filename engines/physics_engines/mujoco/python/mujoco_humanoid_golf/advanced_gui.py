...
class AdvancedGolfAnalysisWindow(QtWidgets.QMainWindow, AdvancedGuiMethodsMixin):
    ...
    def _on_quick_camera_clicked(self, preset_name: str) -> None:
        ...
        if hasattr(self, "visualization_tab"):
            ...
            index = self.visualization_tab.camera_combo.findText(preset_name)
            if index >= 0:
                try:
                    self.visualization_tab.camera_combo.blockSignals(True)
                    self.visualization_tab.camera_combo.setCurrentIndex(index)
                finally:
                    self.visualization_tab.camera_combo.blockSignals(False)
