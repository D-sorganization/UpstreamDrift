function ClubDataGUI_PROMPT_FINAL()
    ...
    for i = 1:6
        handles.axisToggles(i) = uicontrol('Parent', handles.panel, 'Style', 'checkbox', ...
            ...
            'Callback', @(src, ~) animateFrame(handles)); % Update on toggle
    end
    ...
    handles.playBtn = uicontrol('Parent', handles.panel, 'Style', 'togglebutton', ...
        ...
        'Position', [0.1 0.06 0.8 0.05], 'Callback', @(src, ~) togglePlayback(src, handles));
...
function plotSheet(src, handles, COL)
    ...
    handles = guidata(handles.fig); % Get the latest handles!
    ...
    try
        ...
        errordlg(['Error reading data from sheet: ' ME.message]);
        return;
    end

    % Data validation (basic check; improve as needed)
    if size(data, 2) < max([struct2array(COL)])
        errordlg('Insufficient data in the selected sheet.');
        return;
    end
...
function togglePlayback(src, handles)
    ...
    handles = guidata(src);  % Get the latest handles!
    set(src, 'Enable', 'off'); % Disable button during playback
    try
    ...
    set(src, 'Enable', 'on');  % Re-enable the button
end
...
function animateFrame(handles)
    ...
    for j = 1:3
        ...
        % Update quiver plots and visibility based on checkboxes
