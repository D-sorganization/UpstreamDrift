...
def validate_launchers():
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location(
            "launch_golf_suite", 
            Path(__file__).parent / "launch_golf_suite.py"
        )
...
def validate_shared_components():
    ...
    try:
        ...
        from shared.python.common_utils import (
            setup_logging, ensure_output_dir, load_golf_data,
            standardize_joint_angles, convert_units
        )
        logger.info("  ✅ Shared Python utilities import successfully")
        
        # Test shared constants
        from shared.python import (
            SUITE_ROOT, ENGINES_ROOT, MUJOCO_ROOT, DRAKE_ROOT, PINOCCHIO_ROOT
        )
