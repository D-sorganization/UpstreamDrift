    ...
    for epoch in range(int(training.epochs)):
        ...
        for start in range(0, len(order), int(training.batch_size)):
            ...
            for idx in batch_idx:
                ...
                teachers = (
                    teachers_raw if isinstance(teachers_raw, tuple) else (teachers_raw,)
                )
                selected_teacher = np.asarray(teachers[0], dtype=np.float64)
                hidden = model.body(model._encode(sample))
                selected = torch.tanh(model.selected_head(hidden)) * model._scale
                mixture = torch.tanh(model.mixture_head(hidden)) * model._scale
                mixture = mixture.view(
                    1, n_modes, config.u_dim
                ) + model.mode_bias.unsqueeze(0)
                ...
                for mode_i, teacher in enumerate(teachers[:n_modes]):
                    ...
                    mix_loss = mix_loss + nn.functional.mse_loss(mixture[0, mode_i], t)
                mix_loss = mix_loss / float(max(1, min(len(teachers), n_modes)))

                reg = training.control_regularization * selected.pow(2).mean()

                traj_t = torch.tensor(sample.trajectory, dtype=torch.float32)
                mask_t = torch.tensor(
                    sample.conditioning.observation_mask, dtype=torch.float32
                ).reshape(1, -1)
