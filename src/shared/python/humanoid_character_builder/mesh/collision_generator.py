...
class CollisionGeometryGenerator:
    ...
    def _fit_sphere(self, mesh: Any) -> PrimitiveFit:
        ...
        try:
            ...
            radius = float(np.sqrt(np.max(np.einsum("ij,ij->i", vertices, vertices))))  # noqa: E501 ⚡ Bolt: np.sqrt(np.max(np.einsum(...))) is ~2x faster than np.max(np.linalg.norm(..., axis=1))
