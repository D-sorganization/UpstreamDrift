...
class TileHelpRecord:
    """Lightweight tile representation for help resolution."""
...
@dataclass(frozen=True)
class TileHelpRegistry:
    """Registry of tiles loaded from models.yaml."""

    tiles: tuple[TileHelpRecord, ...]

    def get(self, tile_id: str) -> TileHelpRecord | None:
        """Find a tile by ID."""
        for t in self.tiles:
            if t.id == tile_id:
                return t
...
def load_tile_registry(path: Path | None = None) -> TileHelpRegistry:
    """Load tile records from models.yaml."""
    registry_file = path or REGISTRY_PATH
    if not registry_file.is_file():
        return TileHelpRegistry(tiles=())
    with open(registry_file, encoding="utf-8") as f:
        data = yaml.safe_load(f) or {}
    models_raw = data.get("models", [])
    records: list[TileHelpRecord] = []
    for entry in models_raw:
        ...
        status = (
            launcher.get("status", "ready") if isinstance(launcher, dict) else "ready"
        )
        if status == "gui_ready":
            status = "ready"
        maturity = entry.get("maturity", status)
        if maturity == "gui_ready":
            maturity = "ready"
        help_page = entry.get("help")
        if not help_page and isinstance(launcher, dict):
            help_page = launcher.get("help")
        hidden = bool(entry.get("hidden", False))
        records.append(
            TileHelpRecord(
                id=tile_id,
                name=entry.get("name", tile_id),
                help=help_page,
                maturity=maturity,
                hidden=hidden,
            )
        )
    return TileHelpRegistry(tiles=tuple(records))
...
def help_violations(
    ...
    *,
    registry: TileHelpRegistry | None = None,
) -> list[str]:
