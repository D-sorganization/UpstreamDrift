    ...
    *,
    ...
    def residual(p: Array) -> Array:
        ...
        if options.pelvis_indices is not None and options.pelvis_yaw_weight > 0:
            ...
            pred_term = (
                pred_markers[-1] if pred_markers.shape[1] > max(wl_i, wr_i) else None
            )
            obs_term = window_data[-1][3][-1] if len(window_data[-1][3]) > 0 else None
            yaw_res, _, _ = compute_pelvis_yaw_residual_and_derivative(
                pred_markers_term=pred_term,
                target_markers_term=obs_term,
                wl_i=wl_i,
                wr_i=wr_i,
                yaw_weight=options.pelvis_yaw_weight,
            )
            res_parts.append(yaw_res)
    ...
    def analytic_jacobian(p: Array) -> Array:
        ...
        for i, (start, _end, clock, points, observed) in enumerate(window_data):
            ...
            if (
                i == n_windows - 1
                and options.pelvis_indices is not None
                and options.pelvis_yaw_weight > 0
            ):
                wl_i, wr_i = options.pelvis_indices
                obs_term = (
                    window_data[-1][3][-1] if len(window_data[-1][3]) > 0 else None
                )
                cached_window = window_cache.get(i)
                state_bytes = None if state is None else state.tobytes()
                theta_bytes = p[:theta_dim].tobytes()
                if (
                    cached_window is not None
                    and cached_window[0] == theta_bytes
                    and cached_window[1] == state_bytes
                ):
                    pred_term = cached_window[2][-1]
                else:
                    pred_fresh, _ = segmented_forward(p[:theta_dim], clock, state)
                    pred_term = np.asarray(pred_fresh)[-1]

                _, yaw_jac, _ = compute_pelvis_yaw_residual_and_derivative(
                    pred_markers_term=pred_term,
                    target_markers_term=obs_term,
                    wl_i=wl_i,
                    wr_i=wr_i,
                    yaw_weight=options.pelvis_yaw_weight,
                    marker_jac_term=global_marker[-1],
                )
                pelvis_yaw_jac = yaw_jac
        for i, node in enumerate(internal_nodes):
        ...
        if pelvis_yaw_jac is not None:
            parts.append(pelvis_yaw_jac)
        if options.regularization is not None:
    ...
    if options.pelvis_indices is not None and len(target.points) > 0 and is_finite:
        wl_i, wr_i = options.pelvis_indices
        pred_final = (
            unsegmented_pred[-1]
            if unsegmented_pred.ndim == 3
            and unsegmented_pred.shape[1] > max(wl_i, wr_i)
            else None
        )
        target_final = (
            target.points[-1] if target.points.shape[1] > max(wl_i, wr_i) else None
        )
        yaw_metrics = compute_pelvis_yaw_metrics(pred_final, target_final, wl_i, wr_i)
        pelvis_yaw_diff_deg = yaw_metrics.yaw_diff_deg
        pelvis_yaw_error_pct = yaw_metrics.pelvis_yaw_error_pct
    ...
    return MultipleShootingFit(
        ...
        optimality=(
            float(optimum.optimality) if hasattr(optimum, "optimality") else None
        ),
