...
class GroundPlane:
    ...
    def __post_init__(self) -> None:
        ...
        if n.shape != (3,) or not np.isfinite(n).all() or math.sqrt(n.dot(n)) < 1e-12:  # ⚡ Bolt: math.sqrt(dot) is faster than np.linalg.norm for small 1D arrays
        ...
        unit = n / math.sqrt(n.dot(n))  # ⚡ Bolt: math.sqrt(dot) is faster than np.linalg.norm for small 1D arrays
        object.__setattr__(
...
def sphere_ground_contact(
    ...
    speed = float(math.sqrt(tangential.dot(tangential)))  # ⚡ Bolt: math.sqrt(dot) is faster than np.linalg.norm for small 1D arrays
    ...
    for center, velocity in states:
        ...
        for name in names[1:]:
            ...
            normal_diff[name] = max(
                normal_diff[name],
                float(math.sqrt((other.normal_force_n - base.normal_force_n).dot(other.normal_force_n - base.normal_force_n))),  # ⚡ Bolt: math.sqrt(dot) is faster than np.linalg.norm for small 1D arrays
            )
            friction_diff[name] = max(
                friction_diff[name],
                float(math.sqrt((other.friction_force_n - base.friction_force_n).dot(other.friction_force_n - base.friction_force_n))),  # ⚡ Bolt: math.sqrt(dot) is faster than np.linalg.norm for small 1D arrays
            )
    return {
