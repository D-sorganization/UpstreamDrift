"""Club-only double/triple pendulum matching with cold vs retrieval starts (CO-04 #10608).

Consumes driven-model adapters, scores first frame before integration, reports
in-plane and original 3D errors separately, and never treats body-only
reconstruction scores as club match evidence.
"""
...
def _fit_triple(
    ...
    *,
    ...
    # Fixed link lengths: short hub + arm/club from median distances.
    ...
    # Finite-difference v0 from next frame when available.
    if n >= 2:
    ...
    grip0 = np.array(fk0["wrist2"], dtype=np.float64)
    head0 = np.array(fk0["tip"], dtype=np.float64)
    first = _first_frame_rmse(grip0, head0, grip[0], head[0])

    x0 = np.zeros(3 * COEFFS_PER_JOINT, dtype=np.float64)
    bounds_lo = np.full_like(x0, -200.0)
    bounds_hi = np.full_like(x0, 200.0)
    ...
    if hub_mode is HubMode.PRESCRIBED_MOVING_HUB:
        # Prescribe a small translational hub track from mid-hands centroid drift.
...
def match_club_pendulum(request: PendulumMatchRequest) -> PendulumMatchResult:
    ...
    attempts: list[tuple[str, Any]] = [("cold", cold_q0)]
    ...
    blockers = list(_NATIVE_BLOCKERS)
    if request.model_id == _DRIVEN_DOUBLE:
        blockers = [
            "native_g1_qualification_requires_desk_native_receipt",
            "software_contract_fit_is_not_native_g1_evidence",
