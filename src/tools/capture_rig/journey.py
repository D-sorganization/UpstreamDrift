    ...
    def show_details(self) -> None:
        ...
        for view in media.views:
            ...
            for name, path in (view.observation_sets or {}).items():
                detector, evidence = observation_evidence(media.root, path)
                ...
                child.setToolTip(
                    0, "Double-click to review this output and its provenance"
                )
                row.addChild(child)
            if row.childCount() == 0:
                row.addChild(
        ...
        for variant in media.variants:
            ...
            table.addTopLevelItem(match_row)
        table.expandAll()
        table.itemActivated.connect(
            lambda item, _column: (
                self.source_requested.emit(item.data(0, Qt.ItemDataRole.UserRole))
        ...
        try:
            history = read_activity(media.root)
        except (ValueError, OSError) as exc:
                ...
                history_table.addTopLevelItem(
                    QTreeWidgetItem(
                        [
                            ...
                            display_status(action, self.active_id),
                            action.context,
                        ]
                    )
                )
