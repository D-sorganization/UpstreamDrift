...
SECURITY_PATTERNS = [
    (re.compile(r"\bprint\s*\("), "Avoid print(), use logging instead"),
    (re.compile(r"\beval\s*\("), "Security risk: eval() detected"),
    (re.compile(r"\bexec\s*\("), "Security risk: exec() detected"),
    (re.compile(r"\byaml\.load\s*\("), "Security risk: Use yaml.safe_load()"),
    (
        re.compile(r"['\"]sk_live_[a-zA-Z0-9]{24,}['\"]"),
        "Security risk: Potential Stripe Secret Key",
    ),
    (
        re.compile(r"['\"]AKIA[0-9A-Z]{16}['\"]"),
        "Security risk: Potential AWS Access Key",
    ),
    (
        re.compile(r"\bpickle\.load(s)?\s*\("),
        "Security risk: Unsafe deserialization with pickle",
    ),
    (
    ...
    (
        ...
        "Security risk: Use tempfile.mkstemp() instead",
    ),
]
...
def check_security_patterns(
    ...
    """Check for security patterns in code (ignoring comments)."""
    issues: list[tuple[int, str, str]] = []
    # Skip checking quality check scripts
    if filepath.name in [
        "quality_check_script.py",
        "quality_check.py",
        "matlab_quality_check.py",
...
def check_file(filepath: Path) -> list[tuple[int, str, str]]:
    ...
    try:
        ...
        issues.extend(check_security_patterns(lines, filepath))
