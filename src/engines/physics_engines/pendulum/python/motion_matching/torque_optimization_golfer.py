...
class BernsteinGolferTorqueProfile:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            controls.shape != (N_ACTUATED_JOINTS, COEFFS_PER_JOINT)
            or not np.isfinite(controls).all()
        ):
            raise ValueError(
                "controls must be finite with shape "
                f"({N_ACTUATED_JOINTS}, {COEFFS_PER_JOINT})"
            )
    ...
    def curvature_penalty(self, weight: float = 0.05) -> np.ndarray:
        ...
        return bernstein_curvature_penalty(self.controls, weight=weight)

    def effort_penalty(self, weight: float = 0.001) -> np.ndarray:
        """Penalize L2 magnitude of control points."""
        return bernstein_effort_penalty(self.controls, weight=weight, scale=100.0)
