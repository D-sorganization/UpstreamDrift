...
class MinimizeMarkerKinematics(FullBodyMarkerKinematics):
    ...
    @postcondition(
        lambda result, *_a, **_k: np.isfinite(result.q).all(),
    ...
    def solve_pose(
        ...
        **kwargs: Any,
        ...
        def marker_rms(q_k: Array) -> float:
            self._set(q_k)
            positions = self._positions()
            diff = positions - targets_arr
            errors = np.sqrt(np.einsum("ij,ij->i", diff, diff))
            return (
                float(np.sqrt(np.mean(errors[prep.mask] ** 2)))
                if prep.mask.any()
                else 0.0
            )
        ...
        x0 = q_seed[prep.free].reshape(-1, 1)
        ...
        q = q_min if marker_rms(q_min) <= seed_rms else q_seed
        self._set(q)
        positions = self._positions()
        diff = positions - targets_arr
        errors = np.sqrt(np.einsum("ij,ij->i", diff, diff))
