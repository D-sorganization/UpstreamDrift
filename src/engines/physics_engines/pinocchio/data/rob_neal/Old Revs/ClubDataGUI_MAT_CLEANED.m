function ClubDataGUI()
    ...
    try
        ...
        if isempty(dataSheets)
            errordlg('No valid data sheets found.');
            return;
        end
    catch ME
   ...
   % Preset Views (Checkboxes)
    ...
    handles.viewCheckboxes = gobjects(1, length(viewNames)); % Pre-allocate handles
    handles.currentView = 1; % Initialize to 1 (first view)
    ...
    for i = 1:length(viewNames)
        handles.viewCheckboxes(i) = uicontrol('Parent', handles.viewPanel, 'Style', 'checkbox', ...
            ...
            'Callback', @(src, ~) setViewCheckbox(src, handles, viewAngles, i));
    end

    % Select the first view by default AND trigger its callback
    set(handles.viewCheckboxes(1), 'Value', 1);
    setViewCheckbox(handles.viewCheckboxes(1), handles, viewAngles, 1);
...
function setViewCheckbox(src, handles, viewAngles, clickedIndex)
    for i = 1:length(handles.viewCheckboxes)
        if isgraphics(handles.viewCheckboxes(i))
            set(handles.viewCheckboxes(i), 'Value', 0);
        end
    end
    set(src, 'Value', 1);
    view(handles.ax, viewAngles{clickedIndex});
    [az, el] = view(handles.ax);
    set(handles.viewLabel, 'String', sprintf('Az: %.1f°  El: %.1f°', az, el));
    animateFrame(handles);
end
    % setViewCheckbox: Ensures only one view checkbox is selected.

    fig_handle = gcbf;  % Get the handle of the current figure
    try
        handles_fig = guidata(fig_handle); % Get the latest handles structure
        handles = handles_fig;
    catch
        handles_src = guidata(src);
        if isstruct(handles_src) && isfield(handles_src, 'viewCheckboxes') && all(isgraphics(handles_src.viewCheckboxes)) && isscalar(handles_src.Parent) && islogical(isvalid(handles_src.Parent)) && handles_src.Parent
            handles = handles_src;
        else
            % Create a dummy structure with the required fields
            handles = struct();
            if isprop(handles_src, 'Parent') && isscalar(handles_src.Parent) && islogical(isvalid(handles_src.Parent)) && handles_src.Parent
                handles.ax = handles_src.Parent;  % Assuming checkboxes are within the same axes
            if isprop(handles_src, 'Parent') && isscalar(handles_src.Parent) && islogical(isvalid(handles_src.Parent)) && handles_src.Parent
                handles.viewLabel = handles_src.Parent;
            end
            handles.viewCheckboxes = handles_src; % Assign the source as the only element
    ...
    for i = 1:length(handles.viewCheckboxes)
        if isgraphics(handles.viewCheckboxes(i))
            set(handles.viewCheckboxes(i), 'Value', 0);  % Uncheck
        end
    end

    % 2. Select the clicked checkbox
    set(src, 'Value', 1);  % Check the clicked checkbox

    % 3. Apply the view
    az = viewAngles{clickedIndex}(1);
    el = viewAngles{clickedIndex}(2);
    view(handles.ax, az, el);

    % 4. Update view angles display
    [az_current, el_current] = view(handles.ax);
    set(handles.viewLabel, 'String', sprintf('Az: %.1f°  El: %.1f°', az_current, el_current));

    % 5. Update handles.currentView
    handles.currentView = clickedIndex;
    guidata(fig_handle, handles); % Store the updated handles
