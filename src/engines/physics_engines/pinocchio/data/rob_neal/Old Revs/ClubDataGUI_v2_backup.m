...
function ClubDataGUI_v2()
    ...
    % Playback speed
    uicontrol(panel, 'Style', 'text', 'String', 'Playback Speed (Hz):', 'Units', 'normalized', 'Position', [0.1 0.81 0.8 0.05]);
    ...
    handles.viewMenu = uicontrol(panel, 'Style', 'popupmenu', 'String', {'Isometric','Face-On','Down-the-Line','Top-Down'}, 'Units', 'normalized', 'Position', [0.1 0.83 0.8 0.035], 'Callback', @(src,~) updateView(src, handles));

    % Play/Pause
    handles.playBtn = uicontrol(panel, 'Style', 'togglebutton', 'String', 'Play', 'Units', 'normalized', 'Position', [0.1 0.79 0.8 0.035], 'BackgroundColor', [0.2 0.8 0.2], 'Callback', @(src,~) toggleTimer(src, guidata(src)));

    % Frame slider
    handles.frameSlider = uicontrol(fig, 'Style', 'slider', 'Min', 1, 'Max', length(data.time), 'Value', 1, 'Units', 'normalized', 'Position', [0.3 0.01 0.65 0.02], 'Callback', @(src,~) updateFrameSlider(src, guidata(src)));
...
handles.line = plot3(handles.ax, NaN, NaN, NaN, 'k-', 'LineWidth', 2);
    ...
    handles.showTraceHands = uicontrol(panel, 'Style', 'checkbox', 'String', 'Hands Trace', 'Value', 0, 'Units', 'normalized', 'Position', [0.1 0.31 0.8 0.035]);
    handles.showTraceClub = uicontrol(panel, 'Style', 'checkbox', 'String', 'Club Trace', 'Value', 0, 'Units', 'normalized', 'Position', [0.1 0.27 0.8 0.035]);
    handles.showImpact = uicontrol(panel, 'Style', 'checkbox', 'String', 'Impact Marker', 'Value', 0, 'Units', 'normalized', 'Position', [0.1 0.03 0.8 0.035]);
    ...
    handles.showVelocity = uicontrol(panel, 'Style', 'checkbox', 'String', 'Velocity', 'Value', 0, 'Units', 'normalized', 'Position', [0.1 0.23 0.8 0.035]);
    handles.showAcceleration = uicontrol(panel, 'Style', 'checkbox', 'String', 'Acceleration', 'Value', 0, 'Units', 'normalized', 'Position', [0.1 0.19 0.8 0.035]);

    % Add toggles for midhands velocity and acceleration
    handles.showHandVelocity = uicontrol(panel, 'Style', 'checkbox', 'String', 'Hand Velocity', 'Value', 0, 'Units', 'normalized', 'Position', [0.1 0.15 0.8 0.035]);
    handles.showHandAcceleration = uicontrol(panel, 'Style', 'checkbox', 'String', 'Hand Acceleration', 'Value', 0, 'Units', 'normalized', 'Position', [0.1 0.11 0.8 0.035]);

    % Add legend toggle
    handles.showLegend = uicontrol(panel, 'Style', 'checkbox', 'String', 'Show Legend', 'Value', 0, 'Units', 'normalized', 'Position', [0.1 0.14 0.8 0.035], 'Callback', @(src,dummy) toggleLegend(src, dummy));
    ...
    for idx = 1:6
        ...
        row = floor((idx-1)/2);
        xpos = 0.1 + 0.45 * col;
        ypos = 0.66 - spacing * row;  % moved down significantly (~3 lines worth)
    ...
    % Frame info label
    handles.frameLabel = uicontrol(panel, 'Style', 'text', 'String', 'Frame: 1    Time: 0.000 s', 'Units', 'normalized', 'Position', [0.1 0.86 0.8 0.025]);

    % Frame navigation buttons
    uicontrol(panel, 'Style', 'pushbutton', 'String', '< Frame', 'Units', 'normalized', 'Position', [0.1 0.72 0.35 0.035], 'Callback', @(src,~) stepFrame(-1));
    uicontrol(panel, 'Style', 'pushbutton', 'String', 'Frame >', 'Units', 'normalized', 'Position', [0.55 0.72 0.35 0.035], 'Callback', @(src,~) stepFrame(1));

    % Text labels for vector magnitudes
    handles.velClubLabel = text(handles.ax, 0, 0, 0, '', 'Color', 'c', 'FontSize', 8);
    handles.accClubLabel = text(handles.ax, 0, 0, 0, '', 'Color', 'm', 'FontSize', 8);
    handles.velHandLabel = text(handles.ax, 0, 0, 0, '', 'Color', 'g', 'FontSize', 8);
    handles.accHandLabel = text(handles.ax, 0, 0, 0, '', 'Color', 'y', 'FontSize', 8);

    % Ghost shaft line from midhands in negative Z direction
    handles.ghostLine = plot3(handles.ax, NaN, NaN, NaN, 'k--', 'LineWidth', 1.5);

    % Ghost shaft toggle
    handles.showGhost = uicontrol(panel, 'Style', 'checkbox', 'String', 'Ghost Shaft', 'Value', 1, 'Units', 'normalized', 'Position', [0.1 0.07 0.8 0.035]);

    % Shaft tip deflection label
    handles.deflectionLabel = uicontrol(panel, 'Style', 'text', 'String', 'Tip Deflection: -- m', 'Units', 'normalized', 'Position', [0.1 0.09 0.8 0.03]);
