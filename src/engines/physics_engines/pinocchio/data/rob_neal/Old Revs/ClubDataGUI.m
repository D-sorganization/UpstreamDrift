function ClubDataGUI()
    ...
    if ~isfile(defaultFile)
        errorMessage('Data file not found: ' + defaultFile);
        return;
    end
    ...
    try
        ...
        if isempty(dataSheets)
            errorMessage('No valid data sheets found (excluding "Definitions").');
            return;
        end
    catch ME
        errorMessage('Error reading Excel file: ' + ME.message);
        return;
    end
    % --- 2. Create the GUI ---
    ...
    try
        ...
        errorMessage(['Failed to load initial sheet: ' ME.message]);
    end
end
% --- Subfunctions ---
function plotSheet(src, handles, COL)
    ...
    try
        ...
        errorMessage(['Error reading data from sheet "' selectedSheet '": ' ME.message]);
        return;
    end
    % Data validation (basic check; improve as needed)
    if size(data, 2) < max([struct2array(COL)])
        errorMessage(['Data on sheet "' selectedSheet '" has insufficient columns.']);
        return;
    end
    % Extract data
