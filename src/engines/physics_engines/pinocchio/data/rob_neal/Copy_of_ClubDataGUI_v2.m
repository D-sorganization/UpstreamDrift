...
function ClubDataGUI_v2()
    ...
    % Playback speed
    uicontrol(panel, 'Style', 'text', 'String', 'Playback Speed:', 'Units', 'normalized', 'Position', [0.1 0.68 0.35 0.04]);
    handles.speedSlider = uicontrol(panel, 'Style', 'slider', 'Min', 30, 'Max', 480, 'Value', 240, 'SliderStep', [1/450 0.1], ...
        'Units', 'normalized', 'Position', [0.1 0.72 0.35 0.025]);


    handles.viewMenu = uicontrol(panel, 'Style', 'popupmenu', 'String', {'Isometric','Face-On','Down-the-Line','Top-Down'}, 'Units', 'normalized', 'Position', [0.1 0.86 0.35 0.035], 'Callback', @(src,~) updateView(src, handles));

    % Play/Pause
    handles.playBtn = uicontrol(panel, 'Style', 'togglebutton', 'String', 'Play', 'Units', 'normalized', 'Position', [0.1 0.79 0.35 0.03], 'BackgroundColor', [0.2 0.8 0.2], 'Callback', @(src,~) toggleTimer(src, guidata(src)));
    ...
    handles.showTraceHands = uicontrol(panel, 'Style', 'checkbox', 'String', 'Hands Trace', 'Value', 0, 'Units', 'normalized', 'Position', [0.1 0.525 0.35 0.02]);
    handles.showTraceClub = uicontrol(panel, 'Style', 'checkbox', 'String', 'Club Trace', 'Value', 0, 'Units', 'normalized', 'Position', [0.1 0.50 0.35 0.02]);
    handles.showImpact = uicontrol(panel, 'Style', 'checkbox', 'String', 'Impact Marker', 'Value', 0, 'Units', 'normalized', 'Position', [0.1 0.475 0.35 0.02]);
    ...
    handles.showVelocity = uicontrol(panel, 'Style', 'checkbox', 'String', 'Velocity', 'Value', 0, 'Units', 'normalized', 'Position', [0.1 0.425 0.35 0.02]);
    handles.showAcceleration = uicontrol(panel, 'Style', 'checkbox', 'String', 'Acceleration', 'Value', 0, 'Units', 'normalized', 'Position', [0.1 0.4 0.35 0.02]);

    % Add toggles for midhands velocity and acceleration
    handles.showHandVelocity = uicontrol(panel, 'Style', 'checkbox', 'String', 'Hand Velocity', 'Value', 0, 'Units', 'normalized', 'Position', [0.1 0.375 0.35 0.02]);
    handles.showHandAcceleration = uicontrol(panel, 'Style', 'checkbox', 'String', 'Hand Acceleration', 'Value', 0, 'Units', 'normalized', 'Position', [0.1 0.35 0.35 0.02]);

    % Add legend toggle
    handles.showLegend = uicontrol(panel, 'Style', 'checkbox', 'String', 'Show Legend', 'Value', 0, 'Units', 'normalized', 'Position', [0.1 0.325 0.35 0.02], 'Callback', @(src,dummy) toggleLegend(src, dummy));
    ...
    spacing = 0.04;
    startBottom = 0.25 - 0.1;  % Moved down by 0.1
    ...
    for idx = 1:6
        ...
        handles.axisToggles(idx) = uicontrol(panel, 'Style', 'checkbox', 'String', axis_labels{idx}, 'Value', 1, ...
            'Units', 'normalized', 'Position', [xpos, ypos, 0.35, 0.035]);
    end

    % Keyframe jump dropdown
    keyLabels = {'Address', 'TopOfBackswing', 'Impact', 'Finish'};

    handles.keyMenu = uicontrol(panel, 'Style', 'popupmenu', 'String', keyLabels, 'Units', 'normalized', 'Position', [0.1 0.9 0.35 0.035], ...
        'Callback', @(src,~) jumpToKeyframe(src));

    % Frame info label
    handles.frameLabel = uicontrol(panel, 'Style', 'text', 'String', 'Frame: 1    Time: 0.000 s', 'Units', 'normalized', 'Position', [0.1 0.96 0.35 0.025]);

    % Frame navigation buttons
    uicontrol(panel, 'Style', 'pushbutton', 'String', '< Frame', 'Units', 'normalized', 'Position', [0.1 0.79 0.17 0.03], 'Callback', @(src,~) stepFrame(-1));
    uicontrol(panel, 'Style', 'pushbutton', 'String', 'Frame >', 'Units', 'normalized', 'Position', [0.28 0.79 0.17 0.03], 'Callback', @(src,~) stepFrame(1));
    ...
    % Ghost shaft toggle
    handles.showGhost = uicontrol(panel, 'Style', 'checkbox', 'String', 'Ghost Shaft', 'Value', 1, 'Units', 'normalized', 'Position', [0.1 0.295 0.35 0.02]);

    % Shaft tip deflection label
    handles.deflectionLabel = uicontrol(panel, 'Style', 'text', 'String', 'Tip Deflection: -- m', 'Units', 'normalized', 'Position', [0.1 0.27 0.35 0.02]);
