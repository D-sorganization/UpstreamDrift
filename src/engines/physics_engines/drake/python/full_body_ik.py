...
class DrakeFullBodyIK:
    ...
    def __init__(self, specification: Mapping[str, Any] | bytes | str) -> None:
        ...
        for b in bodies:
            ...
            elif b in self.model.metadata["body_links"]:
                link_name = self.model.metadata["body_links"][b]
                self._body_objects[b] = self.model.plant.GetBodyByName(
    ...
    def pose_fn(self, q: Array) -> dict[str, Pose]:
        ...
        self.model.plant.SetPositions(self.model.context, q_arr)
        poses: dict[str, Pose] = {}

        for b, frame in self._frame_objects.items():
            x_wf = self.model.plant.CalcRelativeTransform(
                self.model.context, self.model.plant.world_frame(), frame
            )
            poses[b] = (x_wf.rotation().matrix().copy(), x_wf.translation().copy())

        for b, body in self._body_objects.items():
            x_wb = self.model.plant.EvalBodyPoseInWorld(self.model.context, body)
    ...
    def closure_residuals(self, q: Array) -> Array:
        ...
        x_wa = self.model.plant.CalcRelativeTransform(
            self.model.context, self.model.plant.world_frame(), self.model._closure[0]
        )
        x_wb = self.model.plant.CalcRelativeTransform(
            self.model.context, self.model.plant.world_frame(), self.model._closure[1]
        )
