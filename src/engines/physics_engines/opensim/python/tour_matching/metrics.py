...
def _pelvis_yaw(points: np.ndarray, wl_idx: int, wr_idx: int) -> np.ndarray:
    ...
    # Vector from right waist to left waist
    delta = points[:, wl_idx, :] - points[:, wr_idx, :]
    # Transverse plane: X is forward/strike, Z is lateral (+Z left, -Z right)
    # Yaw angle relative to lateral axis
