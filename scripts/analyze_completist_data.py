...
def generate_report():
    ...
    report_content += "1. Address Critical Incomplete items in `shared/python` and `engines/`.\n"
    report_content += f"2. Fill in missing features marked with {'TO' + 'DO'} in core logic.\n"
    report_content += f"3. Resolve Technical Debt ({'FIX' + 'ME'}) to ensure stability.\n"
    report_content += "4. Add docstrings to public interfaces.\n"

    # Issues to be created section
    report_content += "\n## Issues to be Created\n"
    report_content += "The following critical items block core functionality and require issues:\n\n"
    for item in critical_candidates[:5]:
        report_content += f"- **[CRITICAL] {item['file']}: {item['type']} at line {item['line']}**\n"
