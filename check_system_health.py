...
def main():
    ...
    if docker_ok:
        try:
            ...
            if res.returncode == 0:
                ...
                print_result("Docker Libs (libEGL/XCB)", "FAIL", "Missing dependencies in image")
                print(res.stderr)
        except Exception as e:
             print_result("Docker Libs Check", "FAIL", str(e))
    ...
    for f in files:
        path = root / f
        exists = path.exists()
        print_result(f"File: {f}", "OK" if exists else "FAIL", "Found" if exists else "Missing")
