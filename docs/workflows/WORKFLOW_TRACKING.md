...
| **CI Standard**              | `ci-standard.yml`                | Active   | Core lint/test lane; does not claim full optional-engine coverage. Carries `impact-explorer-web-build` (UD #9484), the producer job for the `/tools/impact-explorer` route: builds the pinned Tools web bundle with `--base=/impact-explorer-app/`, and `repo-structure-gates` runs `scripts/check_declared_route_producers.py` so a declared route always has a producer. |
