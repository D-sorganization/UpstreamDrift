...
| **CI Standard**              | `ci-standard.yml`                | Active   | Core lint/test lane; does not claim full optional-engine coverage. Carries `impact-explorer-web-build` (UD #9484), the producer job for the `/tools/impact-explorer` route: builds and verifies the served, revision-stamped Tools web bundle (UD #9550); `repo-structure-gates` runs `scripts/check_declared_route_producers.py` so each declared route has its producer. |
...
| **Lock Refresh**             | `lock-refresh.yml`               | Active   | Dispatch-only regeneration of `requirements.lock` / `requirements-dev.lock` / `environment.yml` via `make sync-deps` on ubuntu + Python 3.12; opens a PR with the result (UD #9533).                                                                                                                                                                                       |
