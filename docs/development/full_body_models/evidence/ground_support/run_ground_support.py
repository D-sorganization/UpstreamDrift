...
0. Functional hip calibration: relocate hip joints to functional centres (``full_body_spec_hipcal.json``).
1. Ground calibration from lowest toe markers and per-frame stance detection.
2. Address pose: 41-coordinate IK on frame 0 with upper and lower limb markers and stance pins.
3. Lower-limb marker calibration, femur/tibia grid search, and recalibration (``full_body_spec_hipcal_scaled.json``).
4. Full 654-frame IK, zero-phase low-pass, and consistency re-solve.
5. Forward dynamics tracking with computed-torque control, ground reaction, and playback renders.
...
def main() -> None:
    ...
    alignment_old = (
        json.loads(BUILD_RECEIPT.read_text())["pelvis_alignment"][
