...
- **Branch:** feat/10157-mjx-env-tests
- **PR:** #10092 (FB-5, #10069); #10090 (Step 3 merged); #10089 (FB-4, #10068 merged); #10203 (Step 4 cross-engine replays merged); #10218 (HO-1 #10155 merged); #10224 (HO-2 #10156 merged)
- **Paths:** docs/development/full_body_models; src/shared/python/motion_matching/full_body_spec.py; src/shared/python/motion_matching/contact_law.py; src/shared/python/motion_matching/tour_capture_contract.py; src/shared/python/motion_matching/marker_calibration.py; src/shared/python/motion_matching/full_body_ik.py; src/shared/python/motion_matching/visual_skeleton.py; src/shared/python/motion_matching/derivative_resolution.py; src/shared/python/motion_matching/full_body_forward_dynamics.py; src/shared/python/motion_matching/pipeline; tests/unit/motion_matching/pipeline; tests/unit/motion_matching; tests/unit/tools; scripts/config/mjx_env_pins.json; scripts/setup_mjx_env.ps1; scripts/setup_mjx_env.sh
- **Started:** 2026-09-13
- **Last verified:** 2026-09-16 (SELF; HO-2 #10156 merged into main in PR #10224; HO-3 #10157 MJX environment setup and pure JAX functions verified with double-precision parity (< 1e-6 N) against shared contact law; all 6 JAX-gated unit tests pass in ~/.venv-mjx and skip cleanly in standard environment)
- **Summary:** Full-body pipeline handoff (epic #10162). HO-0 (#10186) landed the visual-layer reconciliation. HO-1 (#10155) migrated the ground-support pipeline under `src/shared/python/motion_matching/pipeline/`. HO-2 (#10156) established the Pydantic V2 receipt schema and validator (`receipt_schema.py`) and generated documentation. HO-3 (#10157) delivered cross-platform MJX virtualenv setup scripts (`setup_mjx_env.ps1`, `setup_mjx_env.sh`) reading pinned versions from `scripts/config/mjx_env_pins.json`, refactored differentiable trajectory optimization into pure testable functions, added `requires_jax` marker to `pyproject.toml`, and implemented 6 unit tests in `tests/unit/motion_matching/test_mjx_optimisation.py` covering knot basis, tail freezing mask, contact force parity (< 1e-6 N), weld wrench equilibrium, initial preload, and finite gradients on truncated 12-frame horizon.
- **Next step:** Land HO-3 PR, then proceed to HO-4 (#10158 visual marker playback GIF in pipeline).
- **Evidence:** scripts/config/mjx_env_pins.json, scripts/setup_mjx_env.ps1, scripts/setup_mjx_env.sh, tests/unit/motion_matching/test_mjx_optimisation.py, docs/development/full_body_models/evidence/ground_support/mjx_trajectory_optimisation.py.
