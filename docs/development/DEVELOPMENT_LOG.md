...
- **Next step:** Use the qualified native weld position Jacobian with node retraction, then add rate and acceleration closure derivatives to the derivative-consistent collocation scaffold. The first finite-difference native Drake preflight is deliberately failed and archived; do not extend it blindly. Qualify all three weld levels before using the path as a reaction-eliminated global-sextic initializer and then one bounded forward fit through the qualified batch seam.
