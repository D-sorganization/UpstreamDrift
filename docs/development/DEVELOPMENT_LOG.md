...
- **Next step:** Launch the bounded two-window direct-node SLSQP trial (run78) from exact run73 coefficients with integrated nodes, once-only shared boundary and small budgets, then replay the returned candidate from the original state. Fixture75 passes (marker5.369e-12 m vs uninterrupted73; full closure/node chart qualified). Direct node-chart Jacobian option adds11 RED/GREEN tests;44 root shooting/node regressions plus mypy/Ruff pass. Do not repeat static-node boxes or simply add single-shooting evaluations. Continue alternate-engine tangent/effort/rigid-closure gates separately.
