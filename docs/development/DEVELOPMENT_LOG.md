...
- **Started:** 2026-09-13
- **Last verified:** 2026-09-14 (SELF; FB-5 shooting fit implemented as an iterative-learning pelvis command with replay-in-the-loop; diverges on both captures at gains 0.7 and 0.25 (best stays iteration 0, driver 74.6 mm replay); replay error attributed to pelvis yaw lag (72.7 of 74.6 mm); unit tests pass)
...
- **Next step:** MM-7b (#10109): formulate the downswing as a trajectory optimisation with a differentiable simulator (JaxSim #6647 or MuJoCo MJX): decision variables the tracked reference of every coordinate, cost the replayed marker error plus a zero-moment-point penalty, constraint the contact dynamics; receipt the first driver solve against the 74.6 mm replay baseline.
