...
- **Started:** 2026-09-13
- **Last verified:** 2026-09-16 (SELF; HO-1 #10155 merged into main in PR #10218; HO-2 #10156 receipt schema, validator, test suite, and RECEIPTS.md documentation verified locally; all 26 pipeline unit tests pass)
- **Summary:** Full-body pipeline handoff (epic #10162). HO-0 (#10186) landed the visual-layer reconciliation. HO-1 (#10155) migrated the ground-support pipeline under `src/shared/python/motion_matching/pipeline/`. HO-2 (#10156) established the Pydantic V2 receipt schema and validator (`receipt_schema.py`), validating all 11 committed runs, with strict dot-separated error reporting, `RECEIPTS.md` reference documentation table generation, and automated markdown freshness testing.
- **Next step:** Land HO-2 PR, then proceed to HO-3 (#10157 MJX environment and JAX-gated tests).
- **Evidence:** docs/development/full_body_models/RECEIPTS.md, tests/unit/motion_matching/pipeline/test_receipt_schema.py, tests/unit/motion_matching/pipeline/test_receipts_markdown_freshness.py, docs/development/full_body_models/evidence/ground_support/anthro_driver/receipt.json.
