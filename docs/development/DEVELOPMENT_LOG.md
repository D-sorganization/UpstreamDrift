...
- **Started:** 2026-09-13
- **Last verified:** 2026-09-14 (SELF; closure weld fitted from the address with anatomical wrists (closure_fit.py, --fit-closure); driver result weld orientation from the address does not make the wrist ranges reachable (address 41 mm, IK 68 mm bounded); wrists stay flagged; unit tests pass)
...
- **Next step:** MM-2 (#10104): fit the constant hand-to-club rotation (three axes) jointly from the address, top and impact frames so that the lead wrist cock, flexion and pronation excursions are minimised together, then impose the human ranges; the C3D has no hand markers, so record that the hand frame is observed only through the club.
