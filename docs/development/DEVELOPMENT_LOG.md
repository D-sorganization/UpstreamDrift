...
- **Started:** 2026-09-13
- **Last verified:** 2026-09-14 (SELF; MM-7: compliant 50 kN/m sole and 12 Hz tracked reference adopted for anthropometric documents, reference zero-moment-point diagnostic in receipts, downswing experiment harness; driver dynamics root error 38 mm through impact (was 178), 7-iron 31 (to 1.5 s; 133 at 1.75 s in the follow-through) mm; unit tests pass)
...
- **Next step:** MM-7b (#10109): dynamics filter on the tracked reference: shift the centre-of-mass path by the cart-table relation until `reference_zmp` stays inside the support polygon with a 2 cm margin, re-solve the IK with per-frame centre-of-mass rows, and receipt the driver replay against the 38 mm root error of the compliant-sole baseline.
