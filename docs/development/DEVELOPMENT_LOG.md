...
- **Started:** 2026-09-13
- **Last verified:** 2026-09-14 (SELF; MJX differentiable trajectory optimisation of the tracked reference (export_mjx_package.py, mjx_trajectory_optimisation.py); driver replay to 1.5 s 56.1 to 40.2 mm in the shared plant, pelvis yaw lag at 1.4 s 12.3 to 2.6 deg; follow-through not yet covered; unit tests pass)
...
- **Next step:** MM-7b (#10109): make the MJX plant's follow-through faithful (it diverges after 1.6 s with a small pelvis error and a large marker error; a stiffer weld and 12 substeps change nothing, the joint-by-joint comparison shows the joints track equally and the pelvis yaw response of the contact differs (semi-implicit Euler with contact at the substep start against RK4), so reconcile the contact integration and port-check against the shared plant's 130 mm late window), then run mjx_trajectory_optimisation.py over the full horizon and validate the whole-swing replay against 74.6 mm.
