...
- **Started:** 2026-09-13
- **Last verified:** 2026-09-14 (SELF; neck joint and address-arm constraints on the anthropometric document: full-capture IK 30.2 mm over all markers with the head at 28 mm, left elbow -25 deg, backswing root 7 mm; left elbow pit still outward; unit tests in the touched files pass)
...
- **Next step:** Derive the wrist base rotation from the anthropometric forearm frame (test first) and rerun the address elbow-pit rows so the left pit faces inward without breaking the swing fit (REVIEW.md 9.2).
