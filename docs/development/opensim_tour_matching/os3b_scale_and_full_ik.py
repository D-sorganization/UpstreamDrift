...
def main() -> int:
    ...
    # Stage 1: Segment Scaling
    scale_res = estimate_segment_scales(full)
    scaled_osim = apply_segment_scaling(
        args.osim, scale_res.scale_factors, args.output / "golf_humanoid_scaled.osim"
    )
    receipt["scaling"] = {
        ...
        "scaled_osim_sha256": sha(scaled_osim),
    }
    save()

    # Stage 2: Calibration on subsampled frames
    ...
    unlocked_base = write_model(
        ...
        args.output / "base_scaled_unlocked.osim",
    )
    ...
    def ik_fn(offsets, cap: TourCapture) -> np.ndarray:
        with tempfile.TemporaryDirectory() as tmp:
            osim_tmp = write_model(
                tracking_variant(scaled_osim, offsets, []),
                Path(tmp) / "cal_model.osim",
            )
            dt = (
                float(np.mean(np.diff(cap.time_s))) if cap.frames > 1 else (1.0 / 360.0)
            )
            rate_hz = 1.0 / dt
            trc_tmp = write_trc(cap, Path(tmp) / "cal.trc", rate_hz=rate_hz)
            ...
            for label in cap.labels:
                ...
                task.setWeight(
                    10.0
                    if "Club" in GOLF_HUMANOID_MARKER_BODIES.get(label, "")
                    else 1.0
                )
                task.setApply(True)
    ...
    cal_result: CalibrationResult = calibrate_marker_offsets(
        cal_capture,
        GOLF_HUMANOID_MARKER_BODIES,
        pose_fn,
        ik_fn,
    ...
    final_model_path = write_model(
        ...
        args.output / "golf_humanoid_scaled_tour_markers.osim",
    )

    # Stage 3: Full 654-frame IK
    full_q = ik_fn(cal_result.offsets, full)
    # Coordinate unwrapping across timeline
    for i in range(len(names)):
        if rotational[i]:
            full_q[:, i] = np.unwrap(full_q[:, i])

    # Compute predicted marker positions across all 654 frames
    ...
    # Compute per-frame RMS and per-marker RMS
    ...
    # Stage 4: Five Shared Metrics
    shared = compute_shared_metrics(full, predicted_markers)

    # Save kinematics
    np.savez_compressed(
        args.output / "ik_full_654.npz",
        time_s=full.time_s,
        q=full_q,
    ...
    receipt.update(
        ...
        shared_metrics=shared.as_dict(),
        overall_marker_rms_m=float(np.mean(per_frame_rms)),
        outputs={
            "scaled_model_sha256": sha(scaled_osim),
            "calibrated_model_sha256": sha(final_model_path),
            "ik_npz_sha256": sha(args.output / "ik_full_654.npz"),
            "overlay_gif": gif_name,
        },
    )
    save()
    return 0
