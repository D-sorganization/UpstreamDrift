...
- Change: `vendor/ud-tools` gitlink, `requirements-tools.txt` and `Cargo.toml` pinned
  ...
  `src/shared/python/camera/`); `docs/shared_tools/divergence_inventory.*` regenerated.
  `src/motion_capture/rig/preview_source.py` is now one `SharedSourceAdapter` — the
  rig's `open/read/close` protocol over the Tools `initialize/start_capture/read_frame`
  ABC; `FfmpegPreviewSource` binds it to `shared.python.camera.FfmpegDirectShowSource`.
  The seam points rig → Tools only. `recorder.dshow_device_ref` delegates to the shared
  builder behind the rig's PnP-id precondition (`require_pnp_instance_id`).
- Key decision: the shared package imports `sidekick.lab.mocap`, which the root test
  process keeps unresolvable by design (`tests/conftest.py` caches UpstreamDrift's own
  Sidekick; `test_the_fallback_declines_gaps_inside_a_cluster_upstreamdrift_owns`).
  The launcher bootstrap (`launch_upstream_drift.py`) resolves it from the pinned tree,
  so runtime is unaffected; `recorder.dshow_device_ref` imports on call and the adapter
  checks run in the isolated provider harness
  (`tests/fixtures/reference_calibration/run_checks.py` → `preview_source_checks.py`).
- Behaviour delta, deliberate: the preview command gains `-rtbufsize 256M` (the shared
  builder always sets it; the recorder's live path already did). Every other token is
  pinned equal to the pre-port list in `test_preview_launches_the_shared_raw_frame_command`.
- Verification: `python -I tests/fixtures/reference_calibration/run_checks.py` → 38
  passed; `pytest tests/motion_capture/rig tests/unit/repo_hygiene/test_no_shadow_of_tools_shared.py
tests/unit/repo_hygiene/test_vendored_tools_fallback.py` → 140 passed;
...
- Next: operator runs the rig check, then the PR merges; consider a Tools follow-up to make
  `shared.python.camera/__init__` lazy so the pure builders import without `sidekick.lab.mocap`.
