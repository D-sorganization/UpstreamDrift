...
- Completed:
  ...
  3. Synchronized child copy of `src/shared/python/ai/` from pinned vendor:
     ...
     - Updated `src/shared/python/ai/gui/_panel_tools.py` and `assistant_panel.py` to pass `project_root` and query Wizard before RAG store.
  ...
  5. Regenerated divergence inventory:
...
- Files changed:
  ...
  - `src/shared/python/ai/gui/assistant_panel.py`: forwarded `_project_root` to `register_panel_tools`.
...
- `test_check_spec_changelog_duplicates.py`: PASSED (14/14 passed).
