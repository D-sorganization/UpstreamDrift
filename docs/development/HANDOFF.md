...
- **NM-01 state:** Worktree `C:/Users/diete/Repositories/Worktrees/UpstreamDrift-10616-nm01`, branch `feat/nm01-learning-tasks-freeze`, lease `local`/`nm01-cursor` on #10616. Frozen contracts: forward / inverse / masked-trajectory-to-controls tasks; TB-00-keyed `CheckpointDimensions` (no hard-coded 27×7); benefit experiment `neural-benefit-experiment/1.0.0` with five baselines, seven latency phases, pilot 100/500/2000 + three seeds, break-even fail-closed when savings ≤ 0. Evidence digest `7d93734cf5a5…`. Modules: `tasks.py`, `roster.py`, `experiment.py`. Tests: `test_learning_tasks.py`, `test_benefit_experiment.py` (24 passed). No training executed; compute caps pending timing probe. Checkpoint SELF.
- **Next:** Open/merge NM-01 PR for #10616, then dispatch [NM-02 #10617](https://github.com/D-sorganization/UpstreamDrift/issues/10617) (native dataset labels).
