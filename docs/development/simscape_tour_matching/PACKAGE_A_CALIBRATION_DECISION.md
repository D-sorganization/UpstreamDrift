...
1. **Fixed-Geometry Calibration Does NOT Resolve the Error Floor:**
   ...
   - On the unchanged baseline Simscape model topology (where `Head` and `Back` markers share the single rigid `Hub` body), re-optimizing fixed 3D marker attachments across the training interval ($t \in [0.0, 0.85]\text{ s}$) yields:
     - Training RMS ($0.0 \le t \le 0.85\text{ s}$): drops by $2.62\text{ mm}$ (from $10.28\text{ mm}$ to $7.66\text{ mm}$).
     - Held-out Validation RMS ($0.85 < t \le 1.814\text{ s}$): **increases** by $+0.18\text{ mm}$ (from $21.92\text{ mm}$ to $22.09\text{ mm}$).
     - Full-capture RMS: slight drop of $0.53\text{ mm}$ (from $17.39\text{ mm}$ to $16.86\text{ mm}$).
     - Hub cluster alone: held-out error remains at $43.65\text{ mm}$ (vs original $43.07\text{ mm}$).
   - **Conclusion:** Parameter adjustment of fixed offsets under the single-Hub assumption cannot close the gap to the 35 mm gate.

2. **The Root Cause Is a Topological Rigidity Invariant Failure on `Hub`:**
   ...
   - When Head and Back are evaluated as **independent rigid clusters**, their individual rigid floors drop dramatically:
     - Head cluster alone (`HeadTop`, `HeadFront`, `HeadSide`): **$0.16\text{ mm}$ mean RMS** across all 654 frames (max $0.55\text{ mm}$).
     - Back cluster alone (`BackTop`, `BackLeft`, `BackRight`): **$5.05\text{ mm}$ mean RMS** across all 654 frames (max $11.36\text{ mm}$).
     - Total unconstrained rigid floor across all bodies drops from **$14.59\text{ mm}$ to $3.72\text{ mm}$**.

3. **Analysis of Flagged 1.25–1.55 s Static Solves:**

   - The $84.03\text{ mm}$ error spike at $1.25\text{ s}$ (Frame 450) is caused by **missing target data and numerical search bounds**, not an anatomical impossibility:
     - At $1.25\text{ s}$, `WaistRight` is unobserved (`valid = False`, target coordinates `[0.0, 0.0, 0.0]`).
     - In the diagnostic search formulation, the target yaw vector was computed between `WaistRight` and `WaistLeft`. When `WaistRight` defaulted to origin, the artificial target yaw vector pointed across the room ($172.3^\circ$), while the physical pelvis yaw was $\approx -40^\circ$.
     - Solves 0–5 enforced this spurious $\pm 5\%$ yaw constraint, forcing `TorsoInput` and `HipInputZ` to pin against their local numerical bounds ($\pm 1.0\text{ rad}$ and $\pm 0.25\text{ m}$).
     - When the unconstrained yaw solves (Solves 6 and 7) ran without this artificial constraint, the RMS immediately dropped to **$31.08\text{ mm}$ with 0 active bounds**.
     - Solves at $1.30\text{ s}$ ($32.09\text{ mm}$), $1.40\text{ s}$ ($35.50\text{ mm}$), and $1.45\text{ s}$ ($38.40\text{ mm}$) similarly reflect local bound encounters rather than dynamic failure.

4. **Clear Next Action & Recommendation:**
   - **Do NOT proceed to an expensive 654-frame dynamic control search** on the baseline single-Hub topology expecting $\le 35\text{ mm}$ terminal marker error. The geometric residual floor of the combined head/back cluster ($43\text{–}48\text{ mm}$) mathematically precludes meeting $35\text{ mm}$ under forward dynamics without large unphysical torques or model deformation.
   - **Do NOT silently alter the Simscape reference model.**
   - **Review and Decision Path:**
     - **Option 1 (Topology Equivalence Review):** Introduce an explicit cervical articulation (neck joint with 3 rotational DOFs) between `UpperTorsoBase` and `Head` in both the Simscape model and Pinocchio native model. Notice that `native_geometry_spec_9967.json` already contains physical solid definitions for `"GolfSwing3D_Kinetic/Hips and Torso Inputs/Neck"` ($m = 2.059\text{ kg}$) and `"Head"` ($m = 4.990\text{ kg}$), but they are lumped rigidly into `COMRod` (`Hub`).
     - **Option 2 (Measurement Target Protocol):** Re-evaluate the marker assignment in `two_window_fit_9967_102`. If `Hub` is strictly meant to represent the thoracic spine/torso, head markers should not be tied to `Hub` for thoracic orientation tracking, or head markers must be weighted separately.
     - **Next Technical Step:** Present this decision report to the team. If Option 1 or 2 is formally approved, calibrate the articulated model; otherwise, document the non-rigid compromise floor before initiating bounded 0.90s continuation.
...
| Subsystem / Cluster               | Markers Included                                           |          Baseline Combined Rigid RMS          |         Split Independent Rigid RMS          |       Reduction        |
| :-------------------------------- | :--------------------------------------------------------- | :-------------------------------------------: | :------------------------------------------: | :--------------------: |
| **Hub Combined**                  | BackTop, BackLeft, BackRight, HeadTop, HeadFront, HeadSide | **$28.72\text{ mm}$** (max $55.83\text{ mm}$) |                      —                       |           —            |
| **Back Cluster**                  | BackTop, BackLeft, BackRight                               |                       —                       | **$5.05\text{ mm}$** (max $11.36\text{ mm}$) | **$-23.67\text{ mm}$** |
| **Head Cluster**                  | HeadTop, HeadFront, HeadSide                               |                       —                       | **$0.16\text{ mm}$** (max $0.55\text{ mm}$)  | **$-28.56\text{ mm}$** |
| **Full Capture (All 25 Markers)** | All 10 bodies / 25 markers                                 | **$14.59\text{ mm}$** (max $27.97\text{ mm}$) | **$3.72\text{ mm}$** (max $9.18\text{ mm}$)  | **$-10.87\text{ mm}$** |

Key frame progression:
...
### 2. Audit of Flagged 1.25–1.55 s Static Solves

| Time (s) | Frame | Valid Markers |  Missing Markers  | Solve Type         |      Error (RMS)      | Active Bounds | Note / Root Cause                                              |
| :------: | :---: | :-----------: | :---------------: | :----------------- | :-------------------: | :-----------: | :------------------------------------------------------------- |
