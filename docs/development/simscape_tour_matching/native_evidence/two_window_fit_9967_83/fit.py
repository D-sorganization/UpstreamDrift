"""Bounded two-window direct-node SLSQP continuation 83 from returned run82.

Single documented factor changes versus run82: restart recenters on returned82
(071ded56...), node bound widened to 0.075 (giving 50% more manifold travel from
returned82 while safely respecting chart radius 0.5 where 0.075 * sqrt(42) = 0.486 <= 0.5),
and terminal weight set to 4.0 (16x penalty in sum of squares) to bring terminal RMS
into <= 35 mm compliance while maintaining Whole RMS <= 25 mm. Continuation 82 was:
...
def main() -> None:
    ...
    parser.add_argument("--box-factor", type=float, default=2.0)
