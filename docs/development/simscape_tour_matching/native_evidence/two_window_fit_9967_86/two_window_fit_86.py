"""Bounded two-window direct-node SLSQP continuation 86 from returned run85.

Single documented factor changes versus run85: restart recenters on returned85
(c028972c...), node bound maintained at 0.075 (safely respecting chart radius 0.5
where 0.075 * sqrt(42) = 0.486 <= 0.5), box factor expanded to 4.0 (physical box 8.0,
giving additional parameter freedom along the descent valley),
terminal weight increased to 12.0 (144x penalty in sum of squares, vs 64x in run 85)
to drive terminal RMS from 42.70 mm down toward <= 35 mm compliance while maintaining
Whole RMS <= 25 mm (currently 22.764 mm) and Early RMS <= 12 mm (currently 10.799 mm),
and iteration budget expanded to 35 iterations (max_nfev 105) so SLSQP has ample
headroom beyond the 25-iteration limit hit in run 85. Continuation 85 was:
...
def main() -> None:
    ...
    parser.add_argument("--max-nfev", type=int, default=105)
    parser.add_argument("--node-bound", type=float, default=0.075)
    parser.add_argument("--box-factor", type=float, default=4.0)
    parser.add_argument("--terminal-weight", type=float, default=12.0)
    ...
    try:
        ...
        # The run73 physical box is +-0.2*AMPLITUDE over parent19; keep it exactly.
        assert np.isfinite(args.box_factor) and args.box_factor >= 1
        box = args.box_factor * 0.2 * AMPLITUDE
        lower = -box - delta73_flat
        upper = box - delta73_flat
        snapped = np.clip(delta73_flat, -box, box)
        receipt["restart_roundoff"] = {
            "max_abs_increment_outside_box": float(np.max(abs(snapped - delta73_flat))),
            "note": "Recovered increment of the restart over parent19 versus the run73 box; theta applies to the exact restart coefficients",
        }
        assert receipt["restart_roundoff"]["max_abs_increment_outside_box"] <= 1e-8
        lower, upper = np.minimum(lower, -1e-9), np.maximum(upper, 1e-9)
        ...
        def metrics(pred: np.ndarray) -> dict:
            ...
            return {
                ...
                "score": float(
                    np.sum(error[valid]) + (args.terminal_weight ** 2) * np.sum(error[-1, valid[-1]])
                ),
            }
        ...
        config = {
            ...
            "physical_box_over_parent19": box,
        ...
        terminal_rows = args.terminal_weight * second.marker_jacobian[-1, observed1[-1]].reshape(
            -1, x73.size + dim
        )
        ...
        term0 = args.terminal_weight * (second.replay.markers_m[-1] - points[-1])[observed1[-1]].ravel()
