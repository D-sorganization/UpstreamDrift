"""Bounded two-window direct-node SLSQP continuation 81 from returned run80.

Single documented factor change versus run80: the physical control box over
parent19 is widened by --box-factor (default 2, i.e. +-4 N/Nm instead of the
inherited run73 +-2 N/Nm), because 41 of 81 controls sat on the run73 box at the
returned run80 point. Everything else is identical. Continuation 80 was:
...
def main() -> None:
    ...
    try:
        ...
        assert receipt["restart_roundoff"]["max_abs_increment_outside_box"] <= 1e-9
