"""Bounded two-window direct-node SLSQP trial from the exact run73 candidate.

Stage3 of NEXT_AGENT_CONVERGENCE_EXECUTION.md: exact run73 coefficients with
...
def main() -> None:
    ...
    receipt: dict = {"status": "running", "residual_evaluations": 0, "jacobians": 0}
    ...
    try:
        ...
        def segmented(theta, window, state):
            ...
            return result.replay.markers_m, result.replay.integration.state[-1]
        ...
        def callback(theta, residual, cost):
            ...
            costs.append({"evaluation": receipt["residual_evaluations"], "cost": cost})
