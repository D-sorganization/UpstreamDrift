function report = replay_returned83_r2025b(repo)
%REPLAY_RETURNED83_R2025B Independent R2025b forward replay of returned83.
...
% computes the 5 canonical metrics, and tests parity against Pinocchio replay.
    ...
    evidence_dir = fullfile(repo, 'docs', 'development', 'simscape_tour_matching', 'native_evidence', 'two_window_fit_9967_83');
    ...
    % Coefficients in cand.coefficients are [27 x 7], ordered [A B C D E F G] per joint
    ...
    report.qualification = 'independent_r2025b_cold_replay_returned83';
    report.candidate_sha256 = 'cfaefcaebb6267e0e45ad42d88eed42d518e1cc70553f06c7008462858a8f56a';
    ...
    report.pinocchio_metrics = struct( ...
        'whole_rms_m', 0.023342583164814294, ...
        'early_rms_m', 0.010761504958229986, ...
        'terminal_rms_m', 0.05117455753761318, ...
        'club_cluster_rms_m', 0.03003019053073747, ...
        'pelvis_yaw_error_pct', 10.52636248386616);
    ...
    fprintf('Whole RMS:       %.3f mm (Pinocchio: 23.343 mm)\n', whole_rms_m * 1000);
    fprintf('Early RMS:       %.3f mm (Pinocchio: 10.762 mm)\n', early_rms_m * 1000);
    fprintf('Terminal RMS:    %.3f mm (Pinocchio: 51.175 mm)\n', terminal_rms_m * 1000);
    fprintf('Club Cluster:    %.3f mm (Pinocchio: 30.030 mm)\n', club_cluster_rms_m * 1000);
    fprintf('Pelvis Yaw Err:  %.2f %%  (Pinocchio: 10.53 %%)\n', pelvis_yaw_error_pct);
