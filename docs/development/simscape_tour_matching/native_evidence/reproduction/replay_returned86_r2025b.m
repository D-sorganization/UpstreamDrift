function report = replay_returned86_r2025b(repo)
%REPLAY_RETURNED86_R2025B Independent R2025b forward replay of returned86.
...
% computes the 5 canonical metrics, and tests parity against Pinocchio replay.
    ...
    evidence_dir = fullfile(repo, 'docs', 'development', 'simscape_tour_matching', 'native_evidence', 'two_window_fit_9967_86');
    ...
    % If candidate forces are in world frame, transform translational forces to base joint axes
    if isfield(cand, 'force_frame') && strcmp(cand.force_frame, 'world')
        rot_wb = [1.0, 0.0, 0.0; ...
                  0.0, 0.8660254037844387, 0.5; ...
                  0.0, -0.5, 0.8660254037844387];
        coeffs(1:3, :) = rot_wb * coeffs(1:3, :);
    end
    ...
    report.qualification = 'independent_r2025b_cold_replay_returned86';
    report.candidate_sha256 = 'fb8fab9352ba83dbe1b862eb0804c7f7f57201c3116f473acb5fdfe668ae44d5';
    ...
    report.pinocchio_metrics = struct( ...
        'whole_rms_m', 0.022351978510836715, ...
        'early_rms_m', 0.010723567451166371, ...
        'terminal_rms_m', 0.04014122477633861, ...
        'club_cluster_rms_m', 0.007123858920172498, ...
        'pelvis_yaw_error_pct', 15.125243900548849);
    ...
    fprintf('Whole RMS:       %.3f mm (Pinocchio: 22.352 mm)\n', whole_rms_m * 1000);
    fprintf('Early RMS:       %.3f mm (Pinocchio: 10.724 mm)\n', early_rms_m * 1000);
    fprintf('Terminal RMS:    %.3f mm (Pinocchio: 40.141 mm)\n', terminal_rms_m * 1000);
    fprintf('Club Cluster:    %.3f mm (Pinocchio: 7.124 mm)\n', club_cluster_rms_m * 1000);
    fprintf('Pelvis Yaw Err:  %.2f %%  (Pinocchio: 15.13 %%)\n', pelvis_yaw_error_pct);
