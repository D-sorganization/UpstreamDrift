function replay_native_tolerance_diagnostic(repo, source_path, output_path, tolerance_scale)
...
arguments
    ...
    tolerance_scale (1,1) double {mustBeFinite, mustBePositive} = 1
end
assert(strcmp(version('-release'),'2025b'),'Require R2025b');
assert(~isfile(output_path),'Refuse to overwrite evidence');
assert(tolerance_scale<=1,'Tolerance scale must not loosen the diagnostic');
...
rtol=num2str(1e-10*tolerance_scale,17);
atol=num2str(1e-12*tolerance_scale,17);
set_param(model,'RelTol',rtol,'AbsTol',atol,'MaxStep','0.0001');
...
report=struct('release',version('-release'),'source_path',source_path, ...
    ...
    'requested_settings',struct('Solver',opts.solver,'RelTol',rtol, ...
    'AbsTol',atol,'MaxStep','0.0001'));
