"""Bounded two-window direct-node SLSQP continuation 85 from returned run84.

Single documented factor changes versus run84: restart recenters on returned84
(f01d551d...), node bound maintained at 0.075 (safely respecting chart radius 0.5
where 0.075 * sqrt(42) = 0.486 <= 0.5), box factor expanded to 3.0 (physical box 6.0,
releasing parameters that hit the 5.0 box boundary at run 84 iteration 15),
terminal weight increased to 8.0 (64x penalty in sum of squares, vs 36x in run 84)
to drive terminal RMS from 46.04 mm down to <= 35 mm compliance while maintaining
Whole RMS <= 25 mm (currently 23.002 mm) and Early RMS <= 12 mm (currently 10.808 mm),
and iteration budget expanded to 25 iterations (max_nfev 75) so SLSQP has ample
headroom beyond the 15-iteration limit hit in run 84. Continuation 84 was:
