"""Bounded two-window direct-node SLSQP continuation 84 from returned run83.

Single documented factor changes versus run83: restart recenters on returned83
(cfaefcae...), node bound maintained at 0.075 (safely respecting chart radius 0.5
where 0.075 * sqrt(42) = 0.486 <= 0.5), box factor expanded to 2.5 (physical box 5.0,
releasing parameters that hit the 4.0 box boundary at run 83 iteration 15), and
terminal weight increased to 6.0 (36x penalty in sum of squares, vs 16x in run 83)
to drive terminal RMS from 51.17 mm down toward <= 35 mm compliance while maintaining
Whole RMS <= 25 mm (currently 23.34 mm). Continuation 83 was:
...
def main() -> None:
    ...
    parser.add_argument("--box-factor", type=float, default=2.5)
