...
Currently, this effect is completely missing from the core solver. The module relies on a separate "heuristic" (`compute_gear_effect_spin`) to fake the spin results (see Issue 011), but the _velocity_ result (ball speed) remains incorrect for off-center hits because it behaves as if the impact was center-face (infinite MOI).
