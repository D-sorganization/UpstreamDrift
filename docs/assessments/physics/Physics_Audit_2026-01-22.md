...
- **Issue:** Natural frequency calculation uses the formula for a uniform cantilever beam ($$\omega_n = \beta_n^2 \sqrt{EI/(\mu L^4)}$$) using _average_ EI and mass properties.
