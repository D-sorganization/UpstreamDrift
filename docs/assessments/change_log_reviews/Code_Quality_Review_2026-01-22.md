...
1.  **Enforce Commit Policy:** The `check_commit_policy.py` script (if it exists) must be wired into a pre-receive hook or CI step that _cannot_ be bypassed by "fixes".
