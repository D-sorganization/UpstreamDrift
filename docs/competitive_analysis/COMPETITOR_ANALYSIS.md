...
| **Full Swing** | Full Swing Kit | Radar-based, Tiger Woods Endorsed, High-res Display | ~$5,000 | Premium Consumer/Prosumer |
| **Garmin** | Approach R10 | Portable, Phone Integration | ~$600 | Entry Level / Consumer |
| **Rapsodo** | MLM2PRO | Mobile Launch Monitor, Video, Spin Axis | ~$300 - $700 | Entry Level / Consumer |
| **Uneekor** | EYE XO, QED | Ceiling Mounted, High Speed Cameras | $7,000 - $14,000 | Premium Home Simulator |

### 2. Software/Analytics Platforms
| Competitor | Products | Key Features | Price Range | Market Position |
|------------|----------|--------------|-------------|-----------------|
| **GSPro** | GSPro V2/V3 | 4K Graphics (Unity), Realistic Physics, Open API | $250/year | Rapidly Growing "Sim-first" Favorite |
...
| **TruGolf** | E6 Connect (Parent) | Integrated Simulators | Varies | Commercial/Home |
...
| **Hackmotion** | Wrist Sensor | Wrist Angle Measurement, Biofeedback | ~$300 - $1,000 | Focused Instruction (Hands) |
| **V1 Sports** | V1 Pro | Video Analysis, Drawing Tools, Cloud Storage | Subscription | Traditional Coaching Standard |
| **BodiTrak** | Pressure Mats | Pressure Mapping, Center of Pressure (COP) | ~$2,000+ | Portable Pressure Analysis |
...
| **OpenBiomechanics**| Project Data | Open Data Sets, Pitching/Hitting Data | Free | Research Data Repository |
...
2.  **Key Features:** Dual Radar Technology (OERT), vast data parameters, reliable outdoor tracking, Tracy AI.
...
6.  **Technology Stack:** Radar (Doppler), Proprietary Embedded OS, C++/C# Desktop Software, iOS Apps.
7.  **Recent Updates:** TrackMan iO (Indoor Optimized) to compete with Foresight in smaller spaces.
8.  **Our Differentiation:** We offer transparency in physics models where TrackMan is a black box; we aim for software-defined analysis agnostic of hardware.
...
2.  **Key Features:** Quadrascopic imaging (GCQuad), direct measurement of club face data (fiducials).
...
6.  **Technology Stack:** High-speed Near-Infrared Cameras, FPGA/Embedded processing.
7.  **Recent Updates:** QuadMAX with touchscreen and onboard data storage.
8.  **Our Differentiation:** We focus on the physics *after* the data is captured, potentially correcting or enhancing raw data with advanced modeling (e.g. spin decay).
...
6.  **Technology Stack:** Unity Game Engine (C#), Community-built Course Tools (OPCD).
7.  **Recent Updates:** V2 release with major graphics overhaul.
8.  **Our Differentiation:** We provide the *scientific* backend (physics engine, biomechanics) that could theoretically power a simulation like GSPro; we are open source vs their closed source (though they have an open API).

### Sportsbox AI (Biomechanics)
1.  **Core Value Proposition:** Democratizing 3D motion capture using just a smartphone (Single camera).
...
6.  **Technology Stack:** Computer Vision, Deep Learning (PyTorch/TensorFlow backend), Mobile App Frontend.
7.  **Recent Updates:** Expansion to 2-camera systems for higher accuracy.
8.  **Our Differentiation:** Our biomechanics pipeline is transparent and extensible; we allow researchers to tweak the skeleton and inverse dynamics models, whereas Sportsbox is fixed.
...
6.  **Technology Stack:** Python, Cloud Processing, OpenSim Integration.
7.  **Recent Updates:** Continued integration with SimTK/OpenSim.
8.  **Our Differentiation:** We are building *golf-specific* workflows on top of tools like this, adding the club and ball physics that generic biomechanics tools lack.

---

## Feature Comparison Matrix

| Feature | Our Project | TrackMan | Foresight | GSPro | Sportsbox AI | OpenCap | K-Motion |
|---------|-------------|----------|-----------|-------|--------------|---------|----------|
| **Ball Flight Physics** | **Poly-Engine** (Drag/Magnus) | Measured (Radar) | Measured/Calc (Photo) | Unity Physics (Realistic) | N/A | N/A | N/A |
| **Club Data** | **Modeled/Simulated** | Measured (OERT) | Measured (Dots) | Passthrough | N/A | N/A | N/A |
| **Body Movement** | **In Development** (Video) | N/A (Video only) | N/A | N/A | **Yes (Single Cam)** | **Yes (Multi Cam)** | **Yes (Sensors)** |
| **3D Visualization** | **Yes** (Web/Native/Qt) | Yes (Tracy) | Yes (FSX) | Yes (Course) | Yes (Avatar) | Yes (Skeleton) | Yes (Avatar) |
| **Export/API** | **Full JSON/Python API** | Restricted (SDK $) | Restricted | Open API | Restricted | Open Data | Restricted |
| **Pricing** | **Free / Open Source** | $$$$$ | $$$$$ | $$ | $$ (Sub) | Free | $$$ |
| **Platform Support** | **Cross-Platform (Py)** | Windows/iOS | Windows | Windows (High Spec) | iOS/Android | Web/Cloud | Windows/iOS |
...
5.  **Scientific Rigor:** We prioritize the physics (drag coefficients, spin decay, metabolic cost) over "game-ified" results.
...
5.  **Model Validation Platform:** Serve as the benchmark against which commercial tools are tested (e.g., "Does TrackMan's spin decay model match the physics simulation?").
